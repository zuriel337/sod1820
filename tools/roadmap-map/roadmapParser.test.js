// Tests for roadmapParser.js — run with: node --test tools/roadmap-map/
...
test("meta: recognizes the canonical v5 header and merge SHA", () => {
  const vm = parseRoadmap(loadRoadmap());
  assert.equal(vm.meta.canonical_status, "CANONICAL");
  assert.equal(vm.meta.version_label, "v5");
...
test("workstreams: all 23 canonical WS-* cards are found, each with core fields", () => {
  const vm = parseRoadmap(loadRoadmap());
  assert.equal(vm.workstreams.length, 23);
