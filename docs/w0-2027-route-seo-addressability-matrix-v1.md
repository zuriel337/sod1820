...
| `/world` | B — public 2029 discovery/SEO hub; sitemap/indexable product route; **not** canonical identity for contained Convergences |
...
| `/topic/:slug` | A — public **Convergence** identity/SEO; preserve canonical address; public Hebrew label = `התכנסות`; `topic` path segment is compatibility URL, not product vocabulary |
...
- `/world` is the discovery hub and may list/link many Convergences; it never replaces their canonical identity URL.
- `/topic/:slug` remains Class A through the 2029 renderer migration.
- Do not redirect Topic URLs into `/world?... ` or mint `/world/topic/...` duplicates.
- Legacy Topic rendering stays as compatibility until native Topic 2029 has canonical/meta/robots/sitemap/OG/structured-data/browser parity.
- `/beit-midrash` and `/beit-midrash/:method` remain Class A until a separate Human-Gate inbound/SEO disposition.
- Full execution order and AI/search representation rules: `docs/2029-world-convergence-seo-ai-discovery-plan-v1.md`.
