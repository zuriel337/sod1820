...
- **Updated (post-#187, ZURIEL Human-Gate):** both the **self** scope (`F-1a′`, `fn_upsert_self_profile`) and the **family-member** scope (`F-1b`, one normalized `parent_of` direction, server-minted `person:<owner>:p:<ref>`) are now approved and live — Ledger-only, `research_objects`, `privacy_scope='private'` always, zero `nodes`/`edges` projection (`docs/planning/sql/fn_family_private_slice.sql`). **OD-F8 (graph projection) remains explicitly out of scope and unresolved** — this contract does not reopen it. A companion Claim/Invitation workflow layer (`family_claims`, a dedicated server-only table — never a `research_objects` relation) governs how a family member later attaches their own account to an existing `person-ref`; see `docs/planning/sql/fn_family_claims_phase{1,2}.sql`. First UI consumer: `src/components/PersonJourney.jsx` ("🧭 מסע החיים — יסוד"), admin-only for now — foundation only (self + family + relations), not yet a Finding-traversal Journey.
