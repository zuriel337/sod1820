...
## 🧭 סדר-היסוד האסטרטגי (STRATEGIC FOUNDATION ORDER) — חדש, נוסף 22.8, **תוקן 22.8 (FOUNDATION_ORDER_CORRECTION)**, **עודכן 22.8 (RESEARCH_DNA_V1_FOUNDATION_CONTRACT)**, **תוקן שוב 22.8 (§4 — LEGACY BASELINE · ADDITIVE ACCESS · PREMIUM DEPTH, פאס חמישי וסופי)**
...
**חוזה-היסוד המלא (22.8, לא-build; §4 תוקן 22.8 — פאס חמישי וסופי בשרשרת):** `audits/research_dna_v1_foundation_contract/` — `RESEARCH_DNA_V1_FOUNDATION_CONTRACT.md` (Claim/Calculation/Verification + ממדי-DNA כולל **Approval** חדש ו-**Baseline/Enrichment Provenance** כתת-שדה של Provenance + Preserve&Expand + **§4 מתוקן: LEGACY BASELINE · ADDITIVE ACCESS · PREMIUM DEPTH LAW** — מחליף במלואו את הניסוח הקודם ("Premium controls depth and tooling, not truth" הוחלף ב-**"Premium controls access, depth and tooling — never mathematical truth or canonical status"**; "an approved method's raw computed value is never Premium" הוחלף ב-Method Access Law מדויק: Mathematical Truth תמיד זהה בכל tier, אך Method Access (מי-חשוף-למה) יכול-להיקבע ב-Human-Gate לשיטה חדשה-שאושרה). **חוק-חדש: LEGACY BASELINE LAW** — provenance/metadata חייבים-להספיק כדי-לדעת מה-היה-ב-baseline מול מה-נוסף-לאחר-מכן (ברמת-שורה: כן מספיק היום דרך `created_at`; ברמת-שדה בתוך שורה-קיימת: `IMPLEMENTATION DECISION REQUIRED`, לא-הומצא-פתרון). **חוק-חדש: ADDITIVE ENRICHMENT LAW** — ארבעה צירים אורתוגונליים לעולם-לא-ממוזגים: Classification / Provenance-Generation(baseline-vs-enrichment) / Approval / Access — תוספת ל-world/method קיים אינה יורשת אוטומטית את ה-access-tier של המכולה. **WORLD ACCESS LAW + NEW WORLDS/FACETS** — חברות ב-world קיים אינה מקנה tier אוטומטי; world חדש יכול-לקבל tier מ-רגע-האישור, Human-Gate מחליט, בהתאם ל-`platform_tiers_law` הקיים (6 דרגות) — לא-מומצא tier חדש. תוצאה: sod-דוח-ניהול-עתידי (Baseline/Added/Free/Premium/Pending per world/method) מתועד כ-**requirement בלבד**, לא-נבנה. · `CORPUS_APPROVAL_LIFECYCLE.md` (SOURCE→ENGINE-CALC→VERIFICATION→RESEARCH→HUMAN-GATE→APPROVED, `Engine Verified ≠ Corpus Approved`, `Trusted ≠ Canonical` — ללא-שינוי-מהותי, רק pointer ל-Method Access Law החדש) · `METHOD_LIFECYCLE_ALIGNMENT.md` (ר"ת/ס"ת/רגיל-ישר-והפוך נשארים `candidate` במפורש, לא-מופעלים — ללא-שינוי-מהותי, רק pointer). **⚠️ לא-הוחלט/לא-נבנה כאן:** schema חדש (verdict: `NOT YET`, זהה-לכל-פאס-קודם-בשרשרת; פריט-אחד `IMPLEMENTATION DECISION REQUIRED` נוסף — provenance ברמת-שדה, לא משנה את ה-verdict), כתיבת-32-השורות, aliases, הפעלת-שיטה, deploy, ותיוג-Premium-קונקרטי-לכל-facet-עתידי (per-feature Human-Gate כשייבנה). **זהו-הפאס-החמישי בשרשרת** — פאס שביעי-וסופי (22.8, אותו-יום, מיד-אחריו) הוסיף §8-§25 לחוזה, ר' להלן.

**פאס שביעי־וסופי (22.8, אותו-יום) — FINAL CONTRACT CONSOLIDATION, לפני Human-Gate של צוריאל על PR #166:** `RESEARCH_DNA_V1_FOUNDATION_CONTRACT.md` §8-§25 (Part II) נוספו — עדיין **חוזה-בלבד, לא-build, 0 method-activation, 0 composite-implementation, 0 Numeric-Language-implementation**. תוספות מרכזיות: **Unified Gematria Method Law** (§8, מעגן את חוק-ה-DB החי `gematria_methods_catalog` כ-registry-SSOT יחיד) · **Technical Identity ≠ Display Label** (§9, משולש/קדמי כדוגמת-אב) · **Full Method Profile** לכל מילה מאושרת + **Method Profile Contract** (§10-§11, כולל-פיצול-רביעי-חדש: **Calculation ≠ Storage ≠ Display ≠ Access** — שכבה נפרדת מארבעת-הצירים של §4.3) · **Method Versioning** ו-**Method Baseline** (§12/§16, שני פריטי `IMPLEMENTATION DECISION REQUIRED` נוספים — אותו-פער כמו §4.1, מוכלל לרמת-שיטות) · **Method Families** (§13) · **סטטוס מפורש** לאיק-בכר/אחס-בטע (`discovered`, טרם-נבדקו כמועמד-עצמאי) ולמשולש-מילה/משולש-מילה-הפוך (`candidate`, הכי-קרובים-למוכנות — טעונים רק אימות-הגדרה לפני Human-Gate) (§14) · **Atomic ≠ Composite** עם סט-הקומפוזיטים המלא כולל **משולש-מילה+משולש-מילה-הפוך** (חדש, טרם בחוק-ה-DB החי) (§15) · **Multi-Method Cross** (§18, דרישת-חוזה בלבד ל-`WS-CROSS-ENGINE` העתידי: `A.method_X = B.method_Y`, לא רק `A.ragil=B.ragil`) · **Method Consensus/Convergence** (§19, `Convergence ≠ Fact`) · **Negative Results Preservation Law** (§1.1, חדש — `tested→mismatch` נשמר-לעולם, לא-נמחק ולא-נבדק-מחדש-סתם) · **Numeric Language** מוכנס-במפורש-לחוזה כולל הממצא **776→"שבע שבע שש"** (§20-§22, מ-`era2-strong-numbers`/`era2-numeric-language-phase5` — הקורפוס-הגולמי משחזר-באופן-עצמאי את משולש 75/148/776 שה-generator כבר-קבע) · **NUMBER→LANGUAGE→METHOD→NUMBER** כדרישת-חוזה-עתידית-בלבד (§22) · **Bidim/gematria_words** אושר-מחדש כלא-מתמזגים (§23) · **Future Management View** הורחב מ-Worlds-בלבד גם ל-Methods/Corpus (§24) · **המלצת-מועמדות-לחוק-קנוני** (§25, ללא-כתיבת-nodes/codex בפועל — Human-Gate נדרש). מסמך-חדש: `audits/research_dna_v1_foundation_contract/RESEARCH_DNA_V1_FINAL_CONTRACT_COVERAGE.md` — מטריצת-כיסוי מלאה. **זהו-הפאס-האחרון-בשרשרת הזו לפני Human-Gate של צוריאל** — הצעד-הבא הוא Methods Build #1, לא-עוד-חוזה.
