...
>
> **🔎 עדכון 26.8.2026 (PR Reconciliation Pass, Phase B audit) — Corpus Admission Gate כבר-בוצע על-ענף, ממתין-Human-Gate:** `claude/corpus-admission-foundation-v1` (`9f90136b`) מימש-בפועל את ה-Corpus-Admission-Gate שהפסקה-למעלה מתארת כאופציה-פתוחה — **על-Supabase-חי** (per המוסכמה: DB חי בלי-תלות-בפריסה), לא-ממוזג-ל-git/main. אומת-עצמאית (READ-ONLY, 6 בדיקות-חיות ישירות: RLS-policy, `add_entity`, `fn_corpus_admission_gate`, 7/7 writers מחווטים, `resolve_word_review`, `nodes`/`project_codex` entries) — **המסמך תואם-במלואו את-המימוש-החי, 0 drift נמצא**. `fn_resolve_word_identity` (3-רמות: EXACT/SAFE_NORMALIZED/POSSIBLE_VARIANT) + `fn_corpus_admission_gate` חדשים; `add_entity` הוקשח (SECURITY DEFINER+admin-check, היה-dead-code-חשוף); `gematria_words` INSERT-policy הוגבל-לאדמין; `resolve_word_review` משלים-lifecycle (`visibility_reason='approved_by_admin'`). **0 טבלאות-חדשות, VIP/`p_safe_to_auto`-behavior לא-שונה בכוונה.** **Foundation-Sufficient verdict (מאומת עצמאית, לא-רק-מצוטט מהמסמך):** **כן** לפריפריה+שני-הפונקציות-המוקשחות — 0 נתיב-לגיטימי-נשבר, 0 שינוי-VIP. **לא-עדיין** ל-Writer-3 בקנה-מידה — רעש ב-`find_similar_words` (התאמות-prefix-כוזבות) + VIP/`p_safe_to_auto`-bypasses (מכוון, לא-נסגר) עדיין-פתוחים, מתועדים-בכנות-במסמך-עצמו (§13). **המלצת Human-Gate:** (1) לאשר-הפריפריה+שתי-הפונקציות-כ-Foundation-sufficient ולקנן ל-Master-State/Roadmap; (2) **לא** לאשר-עדיין Writer-3-בקנה-מידה עד-סגירת-שני-הפריטים-הפתוחים; (3) המסמך-עצמו (`docs/corpus-admission-foundation-v1-contract.md`) עדיין-לא-ממוזג — PR-חדש/מיזוג-ישיר נדרש כשצוריאל מאשר. Provenance: `nodes.rule_id='corpus_admission_foundation_v1'` + `project_codex.slug='corpus_admission_foundation_v1'` (שניהם כבר-חיים ב-DB, עודכנו-קודם-לסבב-הזה) · ענף `claude/corpus-admission-foundation-v1`@`9f90136b` · work_log 26.8.2026 (2 רשומות: pass-1 + closure-pass-2).
>
