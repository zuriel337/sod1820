...
- **✅ EXTENSION POINT NOW / LATER IMPLEMENTATION — Personal Hint / Synchronicity Research Lens (27.8.2026, §5-A additive ל-אותו-חוזה, `PERSONAL_HINT_TIME_TANACH_FOUNDATION_EXTENSION`):** לא-דומיין-חדש — הרחבה-אחת (§5-A) של `person_foundation_contract` הקיים. Personal Hint = Lens בתוך Research OS (לא Engine/Store/Graph אישי), מנתב Personal Event/Time-input → representation(s) נפרדים → Universal Research Router → Tanach/Numeric/Gematria/ELS/Spatial lens קיימים → Finding מועמד → Personal Hints projection. Time→Tanach: שעה (למשל 16:41) יכולה-להפיק 16/41/1641 או chapter=16/verse=41 **רק** עם Book/Corpus-Context מפורש — **אסור** לבחור-ספר-אוטומטית-משעה; אין Time/Tanach Engine מקביל, שימוש-חוזר-במנועים-קיימים-בלבד. `PERSONAL HINT≠FACT≠CANONICAL RELATION≠DIVINE SIGN` — AI מחשב/מדרג/מציע, לעולם-לא-מכריז-משמעות-אישית-כאמת. `OD-TIME-8` (Gate #21 למעלה) נשאר **בעלים-יחיד** של מודל-הזמן-הרחב — §5-A תואם-ולא-מתחרה. **`rule_version` 1→2 (DB חי, כבר-בוצע).** **לא Active Build, לא Projection/Experience authorization.** פירוט-מלא: Master State §23.14 — **לא-מוכפל-כאן**. ענף `claude/person-hint-foundation-mirror`, git-mirror-בלבד, טרם-merge.
