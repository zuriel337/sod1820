...
assert.match(curationMark, /העדים/);
...
assert.equal(curationProjection.includes("metadata->>tier\", \"silver"), false, "Silver must remain outside the 2029 public curation catalog until semantics are decided");
assert.equal(curationMark.includes("Human Gate"), false, "public curation explanation must not expose governance jargon");
...
assert.match(deepView, /onOpenHeichal \?/);
