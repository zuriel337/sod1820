...
assert.match(home, /if \(GoldenPreview\) return <Suspense[^>]*><GoldenPreview \/><\/Suspense>;/, "Home2029Page must early-return the preview without altering the default render path");
...
// 10. Preview import stays lazy — the Golden preview page must never enter every /2029 visitor's
//     bundle just because the flag is unused.
assert.match(home, /lazy\(\(\)\s*=>\s*import\("\.\/BeitMidrash2029Page\.jsx"\)\)/, "Home2029Page must lazy-import the Golden preview page, not statically bundle it");
assert.equal(/^import BeitMidrash2029Page from/m.test(home), false, "Home2029Page must not statically import BeitMidrash2029Page");

// 11. Preview surface is never indexed while it lives behind a preview query flag.
assert.match(page, /noindex:\s*true/, "the Golden preview page must mark itself noindex via applySeo");

// 12. Trace steps are read from the canonical Universal Finding envelope shape
//     (projection.dimensions.trace.steps), not a made-up path.
assert.match(page, /trace\?\.projection\?\.dimensions\?\.trace\?\.steps/, "TraceView must read the canonical gematria_method_trace envelope path");
assert.equal(/trace\?\.data\?\.trace\?\.steps/.test(page), false, "TraceView must not read the old, incorrect trace path");

// 13. Opening an expression in Heichal resolves its canonical numeric focus first, the same way
//     Heichal2029Page's own entry point does, instead of fabricating a phrase-typed subject.
assert.match(page, /from "\.\.\/lib\/research\/numberExpressionFocus\.js"/, "must resolve expression focus via the shared canonical helper before opening Heichal");
assert.match(page, /resolveExpressionFocus\(/, "onOpenInHeichal must call the canonical resolveExpressionFocus helper");
assert.equal(/type: "phrase"/.test(page), false, "must not fabricate a phrase-typed Research Context subject for a resolvable expression");
