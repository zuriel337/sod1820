...
const ELS_SUGGESTIONS = ["משיח", "גאולה", "ישראל", "דוד", "אליהו", "תורה", "ירושלים", "אברהם", "משה", "שרה", "נחש"];
...
export function ELSSection({ gated = false } = {}) {
  ...
  const [advOpen, setAdvOpen] = useState(false);    // הגדרות מתקדמות (מתקפל) — מתחילים לא נחשפים אליהן
    ...
    <div ref={sectionRef} id="els" style={{
      ...
      <div style={{ maxWidth: 880, margin: "0 auto" }}>
        ...
        <div style={{
          ...
          {/* ── שלב 1 · חיפוש פשוט (למתחילים) — מילה/שם + גימטריה + כפתור ── */}
          <label style={labelStyle}>מילה או שם לחיפוש  ·  לאשכול הפרד מונחים בפסיק</label>
          <input style={inputStyle} value={target} maxLength={80}
            placeholder="לדוגמה: משיח, דוד, גאולה"
            onChange={e => setTarget(e.target.value)}
            onKeyDown={e => e.key === "Enter" && run()} />
          {/* 🔢 חישוב שם/מילה — מיד למתחילים: רואים את הגימטריה תוך כדי הקלדה */}
          {(() => {
            const first = (target.split(/[,\n]/)[0] || "").trim();
            if (elsNormalize(first).length < 2) return null;
            return (
              <div style={{ color: C.goldDim, fontFamily: F.royal, fontSize: 12.5, marginTop: 7, textAlign: "center" }}>
                גימטריית «{first}» = <b style={{ color: C.goldLight }}>{calcGem(first).toLocaleString("he")}</b>
                <span style={{ color: C.muted }}> · דילוג השווה לגימטריה מסומן ב-✦</span>
              </div>
            );
          })()}
          ...
          {/* הצעות לחיפוש — לחיצה אחת (כולל שמות נפוצים למתחילים) */}
          <div style={{ display: "flex", flexWrap: "wrap", gap: 7, justifyContent: "center", marginTop: 14 }}>
            <span style={{ color: C.goldDim, fontFamily: F.heading, fontSize: 11, letterSpacing: 1, alignSelf: "center" }}>הצעות:</span>
          ...
          {/* 💡 הסבר אשכול — תכונה חזקה, מוסברת מיד (לא מוסתרת במדריך) */}
          <div style={{
            marginTop: 14, padding: "10px 14px", borderRadius: 8,
            background: "rgba(212,175,55,0.06)", border: `1px solid ${C.border}`,
            color: C.muted, fontFamily: F.royal, fontSize: 12.5, lineHeight: 1.7,
          }}>
            💡 <b style={{ color: C.goldBright }}>חיפוש אשכול:</b> הקלידו כמה מונחים מופרדים בפסיק (למשל <button onClick={() => { setTarget("משיח, דוד"); run("משיח, דוד"); }} disabled={searching || gateLocked} style={{ cursor: "pointer", background: "none", border: "none", color: C.goldLight, fontFamily: "inherit", fontSize: "inherit", fontWeight: 800, padding: 0, textDecoration: "underline" }}>«משיח, דוד»</button>) — והמנוע ימצא <b style={{ color: C.goldLight }}>היכן הם נפגשים קרוב זה לזה</b> במטריצה.
          </div>

          {/* ── שלב 2 · הגדרות מתקדמות (מתקפל · סגור כברירת-מחדל) ── */}
          <button onClick={() => setAdvOpen(v => !v)} style={{
            cursor: "pointer", width: "100%", marginTop: 18, background: "transparent", color: C.goldLight,
            border: `1px solid ${C.border}`, borderRadius: 8, padding: "10px 14px",
            fontFamily: F.heading, fontSize: 12.5, fontWeight: 700, letterSpacing: 1, textAlign: "right",
            display: "flex", justifyContent: "space-between", alignItems: "center",
          }}>
            <span>⚙️ הגדרות מתקדמות <span style={{ color: C.goldDim, fontWeight: 600 }}>· טווח-דילוג · כיוון · ספר · תבנית (לא חובה)</span></span>
            <span>{advOpen ? "▲" : "▼"}</span>
          </button>
          {advOpen && (
