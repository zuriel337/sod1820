...
// חולץ מ-CommandCenterTab היתום (AdminPage.jsx, לא-מנותב לשום tab-key חי) — ראה
// COMMAND_CENTER_ATTENTION_CLOSURE Pass 1 §6. מביא רק את מה שעונה ישירות על
// «מה דורש-אותי / למה / כמה-חשוב / כפילות / מה-מטטרון-ממליץ» — לא traffic/demand-analytics
// כלליים (נשארים מחוץ לפאס 1, אין להם UI חי כרגע ולא נוגעים בהם כאן).
// אין WRITE חדש: review()/runScan() קוראים לאותם RPCs קיימים (reviewRecommendation/
// runMetatronRecommend מ-lib/visits.js) שכבר-היו-בקוד הזה, רק לא-מנותבים לשום מסך.
...
export default function MetatronAttention() {
  ...
  // גבולות-Pass-1 (§6): שורות-הפער כאן הן מידע (מה/למה/כמה) — לא ניווט-לטאבי-אדמין-אחרים
  // (אלה לא נגישים מתוך WarRoomTab; אין deep-link ל-/admin?tab=. Rank-Don't-Hide: המספר עדיין מוצג).
  const gapDefs = [
        ...
        <div style={{ ...card, background: "rgba(30,20,8,0.35)", border: "1px solid rgba(224,168,106,0.35)" }}>
          ...
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 10 }}>
            {alertDefs.map(([k, icon, lbl, col]) => {
              ...
              return (
                <button key={k} onClick={scrollToRec} title="פתח → המלצות"
                  style={{ cursor: "pointer", textAlign: "right", display: "flex", alignItems: "baseline", gap: 6, background: "rgba(8,5,2,0.4)", border: `1px solid ${on ? col : C.border}`, borderRadius: 8, padding: "6px 10px" }}>
          ...
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
            {gapDefs.map(([icon, val, lbl], i) => (
              <span key={i} style={{ display: "inline-flex", alignItems: "baseline", gap: 5, padding: "4px 8px", color: C.goldDim, fontFamily: F.body, fontSize: 11.5, lineHeight: 1.7 }}>
                {icon} <b style={{ color: C.goldLight, fontFamily: F.mono }}>{Number(val || 0).toLocaleString()}</b> {lbl}
              </span>
            ))}
          </div>
          <div style={{ color: C.muted, fontFamily: F.body, fontSize: 10.5, marginTop: 8, lineHeight: 1.6 }}>
            «סרוק פערים» הופך מספרים-עשירים-בלי-כרטיס להמלצות-לאישור למטה.
          </div>
        </div>
      )}
