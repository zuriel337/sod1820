...
const innerStyle = (w) => ({
  ...
  right: "auto", bottom: "auto",   // ⚠️ קריטי: מנטרל את inset:0 (right:0) — ב-RTL הוא גובר על left ומזיז את הפרוסה
...
export default function CipherElulBanner() {
  ...
  // מעבר אוטומטי — שהות שונה לכל שקופית: הצופן נשאר הרבה זמן (לא עובר מהר), אלול קצר יותר.
  useEffect(() => {
    const dwell = slide === 1 ? 26000 : 8000;   // הצופן ~26ש · אלול ~8ש
    const t = setTimeout(() => setSlide((s) => (s + 1) % 2), dwell);
