...
import { useSiteUpdates, openSiteUpdates, closeSiteUpdates, setSiteUpdatesUnseen } from "../lib/siteUpdates.js";
...
export default function LiveChannelFeed({ hideFab = false }) {
  ...
  // 🧭 מצב הפתיחה עבר לחנות גלובלית (lib/siteUpdates.js, אותו דפוס כמו numberDrawer.js) —
  // כדי שגם ה-Bottom Bar (וכל מקום אחר) יוכל לפתוח/לסגור בדיוק את אותו חלון קיים.
  const { open } = useSiteUpdates();
  ...
  // 🧭 משדר את ה-unseen האמיתי שכבר מחושב כאן (השורה למעלה) החוצה ל-Bottom Bar —
  // lib/siteUpdates.js הוא רק צינור-שידור, לא מקור/מחשב-נתונים משלו.
  useEffect(() => { setSiteUpdatesUnseen(unseen); }, [unseen]);
  ...
  return (
    <>
      ...
      {!open && !hideFab && (
        <button className="lcf-fab" onClick={() => openSiteUpdates()} aria-label="פתח עדכונים">
      ...
      {open && (
        <>
          {!docked && <div className="lcf-scrim" onClick={() => closeSiteUpdates()} />}
          <aside className={"lcf-panel" + (docked ? " docked" : "")} role={docked ? "complementary" : "dialog"} aria-label="עדכונים חיים">
            <div className="lcf-head">
              ...
              <button className="x" onClick={() => closeSiteUpdates()} aria-label={docked ? "מזער" : "סגור"}>{docked ? "–" : "✕"}</button>
            </div>
            ...
            <div className="lcf-body" ref={bodyRef}>
                  ...
                  return (
                    <div key={(u.id || i) + "" + u.ch} className={"lcf-row " + (ai ? "sent" : "recv")}>
                      <div className={"lcf-b " + (ai ? "sb" : "rb")}>
                        ...
                        {u.to && !u.capMore && <Link to={u.to} className="lcf-ptr" onClick={() => { if (!docked) closeSiteUpdates(); }}>← לצפייה</Link>}
                        {u.capMore && c.to && <Link to={c.to} className="lcf-ptr" onClick={() => { if (!docked) closeSiteUpdates(); }}>→ לעוד עדכוני {c.name} · דף הערוץ</Link>}
              ...
              {items.length > 0 && (
                <div className="lcf-foot">
                  <Link to="/broadcasts" onClick={() => { if (!docked) closeSiteUpdates(); }}>📡 כל העדכונים · מרכז השידורים ←</Link>
