...
import { canonicalMethodPublicLabel, sortMethodsByCanonicalOrder } from "../../lib/presentation/canonicalPresentation.js";
  ...
  return <div className="sod29-number-method-inspector" data-method-inspector={method.methodKey}>
    <div className="sod29-number-method-inspector-head">
      <div>
        ...
        <strong>{canonicalMethodPublicLabel(method)}</strong>
    ...
    {tab === "calc" ? <div className="sod29-number-method-inspector-pane">
      <div className="sod29-number-method-calc-line">
        ...
        <b>{canonicalMethodPublicLabel(method)}</b>
    ...
    {tab === "raziel" ? <div className="sod29-number-method-inspector-pane sod29-number-method-raziel">
      <div className="sod29-number-method-raziel-mini">
        ...
        <div>
          ...
          <strong>✦ {projection.expression} · {canonicalMethodPublicLabel(method)} → {method.computedValue ?? "—"}</strong>
      ...
      <div className="sod29-number-core2029-raziel-actions">
        <button type="button" onClick={() => onRazielAction?.("explain_method", { kind: "method", methodKey: method.methodKey, methodLabel: canonicalMethodPublicLabel(method), resultValue: method.computedValue ?? null })}>הסבר את השיטה</button>
        <button type="button" onClick={() => onRazielAction?.("compare_methods", { kind: "method", methodKey: method.methodKey, methodLabel: canonicalMethodPublicLabel(method), resultValue: method.computedValue ?? null })}>השווה שיטות</button>
        <button type="button" onClick={() => onRazielAction?.("next_research_step", { kind: "method", methodKey: method.methodKey, methodLabel: canonicalMethodPublicLabel(method), resultValue: method.computedValue ?? null })}>מה לבדוק עכשיו?</button>
...
export default function NumberCore2029({
  ...
  const primaryMethods = useMemo(() => (
    sortMethodsByCanonicalOrder(methods)
      .slice(0, 6)
  ), [methods]);
  ...
  return <section className={`sod29-number-core2029 sod29-number-v10 ${compact ? "is-drawer" : "is-page"}`} data-number-core-root={root} style={NUMBER_CORE_PALETTE}>
    ...
    <section className="sod29-number-v10-method-switcher" aria-label="שש שיטות ראשיות">
      ...
      <div className="sod29-number-v10-method-grid">
        {primaryMethods.map((method) => {
          ...
          >
            <span>{canonicalMethodPublicLabel(method)}</span>
            <strong>{method.computedValue ?? "—"}</strong>
    ...
    <section className="sod29-number-v10-stage" data-stage-root={stageRoot} aria-live="polite">
      <header className="sod29-number-v10-stage-head">
        <div>
          <span>{canonicalMethodPublicLabel(active)} · התוצאה הפעילה</span>
      ...
      {stageLoading ? <div className="sod29-number-v10-loading is-inline"><i /> מעדכן את החיבורים של {stageRoot} ברקע…</div> : null}
        <div className="sod29-number-v10-research-grid">
          ...
          <section className="sod29-number-v10-crossing">
            ...
            {stageCrossing ? <>
              <p>{stageCrossing.methods.map((method) => method.methodLabel).join(" · ")}</p>
        ...
        <section className="sod29-number-v10-calculation-card">
          <div>
            ...
            <strong>{projection.expression} · {canonicalMethodPublicLabel(active)} = {stageRoot}</strong>
        ...
        <footer className="sod29-number-v10-stage-actions">
          <button type="button" onClick={() => onRazielAction?.("next_research_step", { kind: "method_result", methodKey: active?.methodKey || null, methodLabel: canonicalMethodPublicLabel(active), resultValue: stageRoot })}>✦ שאל את רזיאל</button>
