...
function Help({title,children}){
  ...
  return <span style={{position:"relative",display:"inline-flex",verticalAlign:"middle"}}>
    <button type="button" onClick={()=>setOpen(v=>!v)} aria-label={`הסבר על ${title}`}
      style={{border:`1px solid ${C.line}`,background:C.paper,width:22,height:22,borderRadius:"50%",fontSize:12,fontWeight:900,cursor:"pointer",color:C.muted}}>ⓘ</button>
    {open&&<span style={{position:"absolute",zIndex:40,top:28,insetInlineEnd:0,width:"min(330px,80vw)",background:C.paper,border:`1px solid ${C.line}`,borderRadius:14,padding:12,boxShadow:"0 14px 40px rgba(0,0,0,.14)",fontSize:12.5,lineHeight:1.65,color:C.ink,textAlign:"right"}}>
...
function SequenceShell({kind,label,seed}){
  const [focus,setFocus]=useState(String(seed??""));
  ...
  return <article style={{border:`1px solid ${C.line}`,borderRadius:18,overflow:"hidden",background:C.dark,color:"#eef3ff"}}>
    ...
    <div style={{padding:"20px 16px",background:"linear-gradient(180deg,#0b0d12,#121723)",fontFamily:"ui-monospace,SFMono-Regular,Menlo,monospace",letterSpacing:3,textAlign:"center"}}>
      ...
      <button onClick={()=>setNote(`בחרת ${focus}; אפשר יהיה להצמיד, להצליב או לשלוח לרזיאל לאחר חיבור מקור ${kind} המאומת.`)}
        style={{border:"1px solid rgba(102,164,255,.8)",background:"rgba(47,109,246,.18)",color:C.onBlue,borderRadius:10,padding:"9px 14px",font:"inherit",fontWeight:900,cursor:"pointer"}}>{focus}</button>
      <span style={{opacity:.32}}> · · · …</span>
    </div>
...
export default function EntityHubGoldenControls({data,relationGroups=[],onLeave}){
  // NUMBER → ENTITY: phrases with an existing canonical entity node open their own hub (data-derived).
  ...
  return <div style={{marginTop:18}}>
    ...
    <section style={{marginTop:18,background:C.paper,border:`1px solid ${C.line}`,borderRadius:20,padding:18}}>
      ...
      <div style={{marginTop:16,border:`1px solid ${C.line}`,borderRadius:16,padding:16,background:C.soft,minHeight:150}}>
        {dna==="expressions"&&<div>
          ...
          <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>{expressions.length?expressions.map((p,i)=>{const t=typeof p==="string"?p:p?.phrase||p?.label;if(!t)return null;const ent=phraseEntities[t]||null;return <Link key={i} to={ent?ent.href:`/number/${encodeURIComponent(t)}`} onClick={()=>onLeave?.(ent?"entity":"number",{entityId:ent?ent.nodeId:t,entityType:ent?"entity":"phrase"})} data-entity-node={ent?ent.nodeId:undefined} style={{textDecoration:"none",background:C.paper,border:`1px solid ${ent?C.gold:C.line}`,borderRadius:13,padding:"10px 12px",color:C.ink,fontWeight:850}}><span style={{fontSize:17}}>{ent?"🔹 ":""}{t}</span><span style={{display:"block",fontSize:11,color:C.gold,marginTop:3}}>= {identity.label} · רגיל{ent?" · מרכז ישות":""}</span></Link>}):<span style={{color:C.muted}}>אין ביטויי רגיל זמינים ב-Projection.</span>}</div>
        </div>}
        {dna==="multi"&&<div>
        ...
        {dna==="worlds"&&<div>
          ...
          <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(190px,1fr))",gap:9,marginTop:10}}>
            {worlds.length?worlds.map(g=><article key={g.world} style={{background:C.paper,border:`1px solid ${C.line}`,borderRadius:13,padding:12}}>
        ...
        {dna==="links"&&<div>
          ...
          <div style={{display:"flex",gap:8,flexWrap:"wrap",marginTop:10}}>{relationGroups.slice(0,8).map(([name,rows])=><span key={name} style={{background:C.paper,border:`1px solid ${C.line}`,borderRadius:999,padding:"8px 11px"}}>{name} · <b>{rows.length}</b></span>)}</div>
    ...
    <section style={{marginTop:18,background:C.paper,border:`1px solid ${C.line}`,borderRadius:20,padding:18}}>
      ...
      <div style={{marginTop:12}}>{seq==="pi"?<SequenceShell kind="π" label="π · Digital Scrubber" seed={identity.label}/>:<SequenceShell kind="Fibonacci" label="Fibonacci · Sequence Explorer" seed={identity.label}/>}</div>
    </section>
    ...
    <section style={{marginTop:18,display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(260px,1fr))",gap:12}}>
      <div style={{background:C.paper,border:`1px solid ${C.line}`,borderRadius:18,padding:16}}><div style={{fontWeight:900,fontSize:17}}>🔮 התכנסויות חזקות <Help title="התכנסויות">המספר אינו הסיפור היחיד; מציגים קודם מה נפגש ולמה, והמספר/שיטה משמשים ציר הסבר. Quality הוא ranking, לא אמת.</Help></div>
        <div style={{fontSize:30,fontWeight:950,marginTop:8}}>{topics.length}</div>
        <div style={{display:"grid",gap:7,marginTop:8}}>{topics.slice(0,3).map(t=><Link key={t.id||t.slug} to={`/topic/${encodeURIComponent(t.slug)}`} style={{textDecoration:"none",color:C.ink,border:`1px solid ${C.line}`,borderRadius:11,padding:"9px 10px",background:C.soft}}>
          <div style={{display:"flex",gap:8,alignItems:"center",justifyContent:"space-between"}}><b style={{fontSize:14}}>{t.title||t.slug}</b>{t.quality!=null?<span style={{fontSize:10.5,color:C.gold}}>rank {t.quality}</span>:null}</div>
          {t.subtitle?<div style={{fontSize:11.5,color:C.muted,marginTop:3}}>{t.subtitle}</div>:null}
        </Link>)}</div>
      </div>
      <div style={{background:C.paper,border:`1px solid ${C.line}`,borderRadius:18,padding:16}}><div style={{fontWeight:900,fontSize:17}}>🧭 מסעות <Help title="מסע">מסע הוא traversal של אותו Research Context, לא עותק של ידע. כשהמספר חלק ממסע הוא צריך לבלוט.</Help></div><div style={{fontSize:30,fontWeight:950,marginTop:8}}>{journey?1:0}</div>
        {journey?<div style={{marginTop:7}}>
          <div style={{fontSize:12.5,color:C.muted}}>Number Knowledge Journey מחובר. תחנות זמינות:</div>
          <div style={{display:"flex",gap:6,overflowX:"auto",marginTop:7}}>{(journey.branches||[]).slice(0,6).map((b,i)=><span key={i} style={{whiteSpace:"nowrap",border:`1px solid ${C.line}`,borderRadius:999,padding:"6px 9px",fontSize:11.5}}>{b.label||b.value||b.kind||`תחנה ${i+1}`}</span>)}</div>
        </div>:<div style={{color:C.muted,fontSize:12.5}}>אין מסע קנוני זמין כרגע; לא ניצור אחד מלאכותית.</div>}</div>
