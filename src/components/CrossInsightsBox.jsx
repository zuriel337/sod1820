...
// לחיצה מובילה לטאב "חידושי הצלבות" בעולם (שם מתאפס המהבהב). props: light · max.
  ...
  return (
    <div style={{ background: L.panel, border: `1px solid ${L.line}`, borderRadius: 16, padding: "14px 16px", direction: "rtl" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap", marginBottom: 4 }}>
        ...
        <Link to="/world" style={{ marginInlineStart: "auto", textDecoration: "none", color: L.gold, fontFamily: F.heading, fontSize: 12.5, fontWeight: 700 }}>כל ההצלבות →</Link>
      </div>
      {latest && <div style={{ color: L.sub, fontFamily: F.body, fontSize: 11.5, marginBottom: 10 }}>עודכן לאחרונה · {crossDate(latest)}</div>}

      <div style={{ display: "grid", gap: 8 }}>
        {shown.map(c => (
          <Link key={c.id} to="/world" style={{ textDecoration: "none", display: "block", background: L.chip, border: `1px solid ${L.line}`, borderRadius: 12, padding: "9px 12px" }}>
