...
import BottomBar from "./BottomBar.jsx"; // 🧭 Bottom Bar — Experience Shell קבוע, מרכז launchers ליכולות קיימות
...
export default function Layout() {
  ...
  return (
    <div data-theme={mode} data-stream={stream || "none"} style={{ background: dark ? C.bg : P.pageBg, minHeight: "100vh", color: dark ? "#ede4d3" : P.ink, fontFamily: F.body, fontSize: 16, position: "relative" }}>
      ...
      {/* 🧭 מגירת-המספר: הבועה הצפה הישנה תמיד מוסתרת — הפעולה שלה עברה ל-Bottom Bar (כפתור «🧮 מספר»).
          המגירה עצמה (ה-panel) לא השתנתה — עדיין נפתחת/נסגרת דרך lib/numberDrawer.js מכל מקום באתר. */}
      <NumberDrawer hideLauncher />
      {/* 🧭 חלון-העדכונים: ה-fab הצף הישן מוסתר — הפעולה עברה ל-Bottom Bar (כפתור «📣 עדכונים»),
