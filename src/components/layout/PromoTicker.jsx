...
// 🎗️ טיקר-פרומו מלכותי — בסגנון סרגל-האנגלית, אבל מתחלף כל כמה שניות בין 3 פרומואים «בקרוב»:
...
// יחיד, גלובלי, ניתן-לסגירה (localStorage). מחליף את הטיקרים הישנים (חוץ מסרגל-האנגלית).
...
export default function PromoTicker() {
  ...
  const start = useMemo(() => Math.floor(Math.random() * PROMOS.length), []);   // «מתחלף כל פעם» — התחלה אקראית
  ...
  return (
    <div dir="rtl" role="region" aria-label="בקרוב באתר"
      style={{
      ...
      {/* תג «בקרוב» */}
      <span style={{ flex: "0 0 auto", background: "#e8c84a", color: "#241247", fontFamily: "sans-serif", fontWeight: 900, fontSize: 12, letterSpacing: 1, padding: "3px 11px", borderRadius: 999, animation: "promo-pulse 2.2s ease-in-out infinite" }}>בקרוב</span>
      ...
      <span aria-hidden style={{ position: "absolute", insetInlineStart: 12, bottom: 5, display: "inline-flex", gap: 4 }}>
        {PROMOS.map((_, i) => (
          <span key={i} style={{ width: i === idx ? 14 : 6, height: 4, borderRadius: 999, background: i === idx ? "#e8c84a" : "rgba(255,255,255,.3)", transition: "all .25s" }} />
        ))}
      </span>
