...
export default function ResearcherBadge({ name, display, uid, size = 20, showAvatar = true, style, nameStyle }) {
  const P = usePalette();
  const level = useResearcherLevel(uid);
  const nm = (name || "").trim();
  if (!nm) return null;
  const shown = (display || "").trim() || nm;   // 🌳 עץ אחד: שם-התצוגה שהמשתמש בחר, קישור יציב לפי name
  const initial = shown.charAt(0);
  return (
    <span style={{ display: "inline-flex", alignItems: "center", gap: 6, flexWrap: "wrap", ...style }}>
      ...
      <ResearcherLink name={nm} display={display} style={{ color: P.accentText, fontWeight: 800, textDecoration: "none", ...nameStyle }} />
      {level?.label && (
