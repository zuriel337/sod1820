...
export function normMethod(s) {
  ...
  // ⛔ Foundation Closure (Method Identity, 26.8.2026): "ריבוע" ו-"משולש" הם method_key **שונים** בלוח
  // gematria_methods החי (fn_ribua מול קדמי/משולש-מילה/משולש-הפוך/משולש-גדול/משולש-מדרגות) — המיזוג הישן
  // "ריבוע|משולש"→"ריבוע" היה שגוי-בפועל, לא רק גס. "משולש" עצמו נשאר עמום-קנוני מכוון (5 method_key חיים
  // נושאים "משולש" ב-display_label; קדמי הוא היחיד עם alias מאושר — display_label="קדמי · משולש") — לא
  // ממציאים ברירת-מחדל כאן; "משולש" חוזר כפי-שהוא (החוזה המתועד של הפונקציה: "לא-מזוהה נשאר כמו-שהוא").
  if (/ריבוע/.test(t)) return "ריבוע";
