...
export function appendDeepFacts(baseFacts, cross) {
  let f = baseFacts || "";
  // 🔗 מפגש רב-שיטתי — עובדה ניטרלית: בלי "הכי חזק", בלי קביעה (החלטת צוריאל 12.7).
  if (cross?.topLine) f += ` זוגות שנפגשים בכמה שיטות (עובדת-מנוע — נקודת-עניין, לא קביעה שזו ההצלבה החזקה): ${cross.topLine}. אם עולה מהם חוט משותף — הצע אותו כהזמנה למחשבה.`;
  ...
  if (cross?.crossLine)   f += ` הצלבות בין-שיטתיות (הערכים = עובדה מהמנוע; סוגי-היחס 🪞🌗🔍 = המודל הפרשני של סוד 1820): ${cross.crossLine}.`;
