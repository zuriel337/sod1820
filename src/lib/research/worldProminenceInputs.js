...
/**
 * Bounded read adapter for World prominence dimensions that are already live but not yet carried
 * by the base EntityHub projection. It is subordinate to the existing readers and RLS: no write,
 * no bypass, no rank persistence, no truth inference, no new store/registry/engine.
 */
export async function fetchWorldProminenceInputs(data, { eventResearchLimit = 20 } = {}) {
  if (!data?.identity) {
    return {
      ...
      eventContext: null,
      access: { researchSupplements: true, crossMethodStrength: true, eventContext: true },
    };
  }
