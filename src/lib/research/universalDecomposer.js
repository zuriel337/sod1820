...
function cleanTerm(value) {
  ...
  // Source shorthand such as ה"זאת" means quoted subject זאת with the article outside the quote.
  term = term.replace(/^ה["״']([^"״']+)["״']$/, "$1");
  return clean(term);
}
...
export function extractQuotedSegments(text) {
  ...
  for (const re of patterns) {
    ...
    while ((m = re.exec(input))) {
      out.push({ kind: "quoted_span", text: clean(m[1]), start: m.index, end: m.index + m[0].length, provenance: "source_exact_span" });
    }
  }
  return uniqueBy(out, (x) => `${x.text}|${x.start}`);
}

// Deliberately conservative: v1 recognizes parenthesized source-native Tanach-style locators only.
// This avoids false positives from ordinary Hebrew prose. Exact witness verification belongs downstream.
...
// Extract only relationships explicitly asserted by the source. Never creates an Edge.
export function extractSourceRelations(text) {
  ...
  const directionalPatterns = [
    {
      ...
      re: /["'״׳]?([^\n,.;:()]{1,35}?)["'״׳]?\s+(?:הוא\s+|היא\s+)?כי?נוי\s+ל(?:ספירת\s+)?["'״׳]?([^\n,.;:()]{1,35})/g,
    },
    {
      cue: "בחינת",
  ...
  // Parenthetical apposition is intentionally symmetric/ambiguous. Human Gate resolves direction.
  const apposition = /([א-ת][א-ת״׳"'\-]{1,24})\s*\(\s*([א-ת][א-ת\s״׳"'\-]{1,30})\s*\)/g;
  let a;
  while ((a = apposition.exec(input))) {
    if (/[,.:]/.test(a[2])) continue; // not a verse locator
    const r = relation(a[1], a[2], "apposition_candidate", "סוגריים", a.index, a[0]);
    if (r) out.push(r);
  }

  // Textual equality asserted by source. Claim only; canonical engine may later verify computable parts.
  const equals = /["'״׳]?([^\n=]{1,45}?)["'״׳]?\s*=\s*["'״׳]?([^\n=]{1,45})/g;
  let e;
...
export function extractProcedureChains(text) {
  ...
  for (const line of input.split(/\n+/)) {
    ...
    out.push({
      ...
      raw: clean(line),
      steps: steps.map((label, index) => ({ index: index + 1, label })),
      provenance: "source_declared_sequence",
      verificationState: "not_tested",
      governanceState: "candidate",
    });
  }
...
function normalizeGraphMatches(graphMatches = {}) {
  ...
  for (const [term, rows] of Object.entries(graphMatches || {})) {
    for (const row of rows || []) out.push({ term, nodeId: row.id, nodeType: row.type, label: row.label, identityKey: row.identity_key || null, active: row.is_active !== false, weight: row.weight ?? null });
  }
  return out;
}
...
function normalizeResearchMatches(researchMatches = {}) {
  ...
  for (const [term, rows] of Object.entries(researchMatches || {})) {
    for (const row of rows || []) out.push({ term, researchObjectId: row.id, statement: row.statement, source: row.source, sourceRef: row.source_ref, status: row.status, engineVerified: row.engine_verified === true, kind: row.kind || null });
  }
  return out;
}
...
export function buildUniversalDecomposition({ source = {}, text = "", analysis = null, expressionBoundary = null, canonicalGematriaFindings = [], graphMatches = {}, researchMatches = {}, time = null, aiInterpretation = null } = {}) {
  ...
  for (const r of relations) {
    if (!graphTerms.has(r.left) || !graphTerms.has(r.right)) unresolved.push({ kind: "relation_identity_resolution", label: `${r.left} ↔ ${r.right}`, reason: "לפחות צד אחד עדיין לא נפתר לזהות גרף קיימת. Relation Candidate בלבד." });
  }
  ...
  return {
    ...
    invariants: [
      ...
      "PROCEDURE_REUSES_EXISTING_PRIMITIVES_BEFORE_ANY_NEW_ENGINE",
    ],
  };
}
