...
// Single "what matters most right now" focal for the hero, sourced only from the already-built
// System Now projection above (itself pure getSystemEvents() shape) — no second feed/engine.
// Convergence is the engine's own "most important" tier (✨), so it wins when present; growth is
// the honest fallback. Returns null rather than fabricate a focal when live state has neither.
export function resolveHeroFocal(systemNow) {
  if (!systemNow) return null;
  const conv = systemNow.topConvergence?.[0];
  if (conv && Number.isFinite(conv.number)) {
    const eventCount = Array.isArray(conv.events) ? conv.events.length : 0;
    return {
      kind: "convergence",
      label: `התכנסות סביב מספר ${conv.number}`,
      detail: eventCount > 0 ? `${eventCount} אירועים ממקורות שונים` : null,
    };
  }
  const growth = systemNow.topGrowth?.[0];
  const growthLabel = growth?.title || growth?.type || null;
  if (growth && growthLabel) {
    return {
      kind: "growth",
      label: growthLabel,
      detail: growth.type && growth.title ? growth.type : null,
    };
  }
  return null;
}
