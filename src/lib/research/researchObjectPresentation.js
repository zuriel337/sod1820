...
function inferStatementLanguage(statement) {
  ...
  // Hebrew-only is safe to infer for the research statement. Latin-only is deliberately NOT
  // inferred as English: it may be a formula, transliteration, code key or another language.
...
function variantForLocale(variants, locale) {
  ...
  for (const candidate of [...new Set([requested, base])]) {
    ...
    // A source label alone is useful provenance copy, but it does NOT make the research object's
    // primary presentation normalized. Require actual human title/summary for locale resolution.
 ...
 *     source_witness_lang_basis: "declared_by_intake" | "inferred_hebrew_only" | "unknown" | null,
 *     variants: {
...
export function resolveResearchObjectPresentation(row, { locale = "he" } = {}) {
  ...
  const title = humanTitle || rawStatement || `Research object ${row?.id || ""}`.trim();
