...
export async function composeResearchW2({
  ...
  nextActions = [],
  // Optional canonical composition seam. It receives ONLY the already access-filtered Bundle.
  // Domain engines remain owners of their facts; this callback may compose meaning/calibration
  // but may not calculate replacement truth or see the raw authorization context.
  synthesizer = null,
  signal = null,
} = {}) {
  ...
  try {
    ...
    // It never receives authorizationContext, executor-native rows, or pre-filter findings.
