...
test('golden 337 occurs first at pi position 230 and remains a traversable noncanonical candidate', async () => {
  ...
  // Reconciliation: the Universal Finding envelope no longer asserts stage (INVARIANT PR1 —
  // a projection adapter does not own epistemic type); it does explicitly declare
  // verification_state:"not_tested" (HG-3), matching the gematria/ELS adapter pattern.
  assert.equal(routed.universal_findings[0].stage, null);
...
test('Fibonacci routes through the same Router and exposes index 13 as a derived numeric root', async () => {
  ...
  assert.equal(routed.universal_findings[0].verification.engine_method_tested, 'fibonacci');
});
