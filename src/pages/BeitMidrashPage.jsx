...
// ✍️ הגשת חידוש — «דף חלק» (בקשת צוריאל): נייר ריק + שם + כפתור שלח.
// המתכונת הקנונית חיה ב-<SubmitChidush> (canonical_ui_components_law) ומרונדרת זהה בבית-המדרש, בפורום ובהיכל.
function SubmitTab() {
  return <SubmitChidush />;
}
...
function Sod1820Tab() {
  ...
  useEffect(() => {
    ...
    supabase.from("gematria_words").select("phrase").eq("ragil", 1820).eq("is_verified", true).eq("is_published", true).limit(80)
...
const CORE = new Set([1820, 358, 1237, 26, 541, 776, 878, 1202]); // מספרי ליבה — פנינים גדולות יותר (878=גוף המשיח · 1202=התגלות משיח)
          ...
          {NUM_LIST.map((n, i) => {
            ...
            return (
              <div key={n} style={{ position: "relative", display: "flex", justifyContent: "center", width: "100%" }}>
                ...
                <button onClick={() => { setNum(n); setTerm(""); }} title={KEY_NUMBERS[n] || ""} style={{
...
// 📐 ספריית שיטות הגימטריה — הסבר + דוגמה חיה לכל אחת מ-השיטות הזמינות.
const METHOD_INFO = {
