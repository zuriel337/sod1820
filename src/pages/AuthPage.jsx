...
export default function AuthPage() {
  ...
  const danger = statusTone(P, "danger");
  ...
  const buttonStyle = (tone, extra = {}) => ({
    width: "100%", display: "inline-flex", alignItems: "center", justifyContent: "center", gap: 10,
    background: tone.background, color: tone.color, border: `1px solid ${tone.border}`, borderRadius: 8,
    padding: "12px 16px", cursor: tone === disabledTone ? "wait" : "pointer", fontFamily: F.ui, fontSize: 14, fontWeight: 700,
    minHeight: 44, ...extra,
  });
  const linkBtn = {
    background: ghostTone.background, border: "none", color: ghostTone.color, cursor: "pointer",
    fontFamily: F.ui, fontSize: 13, marginTop: 18, textDecoration: "underline",
  };
  const inputBase = {
    width: "100%", padding: "12px 14px", background: P.cardSoft, color: P.ink,
    borderRadius: 8, fontFamily: F.body, fontSize: 16, outline: "none", boxSizing: "border-box",
  };
      ...
      <div style={{ background: P.pageBg, minHeight: "100vh" }}>
        <div style={{ direction: "rtl", maxWidth: 460, margin: "0 auto", padding: "72px 24px 120px" }}>
          <div style={{ ...card, textAlign: "center" }}>
            ...
            <div style={{ color: P.ink, fontFamily: F.ui, fontSize: 22, fontWeight: 700, marginTop: 16 }}>{name}</div>
            <div style={{ color: P.inkSoft, fontFamily: F.ui, fontSize: 12, marginTop: 4 }}>
  ...
  const submitTone = busy ? disabledTone : primary;
  const secondaryTone = busy ? disabledTone : secondary;

  return (
    <div style={{ background: P.pageBg, minHeight: "100vh" }}>
    <div style={{ direction: "rtl", maxWidth: 460, margin: "0 auto", padding: "64px 24px 120px" }}>
      <div style={{ textAlign: "center", marginBottom: 26 }}>
        <img src={LOGO_URL} alt="SOD1820" style={{ height: 56, width: "auto", filter: `drop-shadow(0 0 18px ${P.glow})` }} />
        <h1 style={{ color: P.ink, fontFamily: F.display, fontSize: 26, fontWeight: 700, margin: "16px 0 6px" }}>כניסה להיכל</h1>
        <div style={{ color: P.inkSoft, fontFamily: F.body, fontSize: 13 }}>כניסה / הרשמה חינם — קבלו גישה לכלים ולסודות.</div>
      </div>
        ...
        {step === "code" ? (
          <form onSubmit={verifyCode}>
            <div style={{ textAlign: "center", marginBottom: 16 }}>
              ...
              <div style={{ color: P.ink, fontFamily: F.ui, fontSize: 17, fontWeight: 700, marginBottom: 6 }}>הזינו את הקוד מהמייל</div>
              <div style={{ color: P.inkSoft, fontFamily: F.body, fontSize: 13.5, lineHeight: 1.7 }}>
                שלחנו קוד בן 6 ספרות אל <b style={{ color: P.ink }} dir="ltr">{email}</b>
              </div>
            </div>
            <input
              ...
              style={{ width: "100%", padding: "14px", background: P.cardSoft, color: P.ink, textAlign: "center",
                letterSpacing: 10, fontSize: 26, fontFamily: F.numeric,
                border: `1px solid ${err ? danger.border : P.borderStrong}`, borderRadius: 8, outline: "none", boxSizing: "border-box" }}
            />
            {err && <div style={{ color: danger.color, fontSize: 12, marginTop: 6, fontFamily: F.ui, textAlign: "center" }}>{err}</div>}
            <button type="submit" disabled={busy} style={buttonStyle(submitTone, { marginTop: 14 })}>
          ...
          <form onSubmit={loginPassword}>
            <div style={{ textAlign: "center", marginBottom: 16 }}>
              ...
              <div style={{ color: P.ink, fontFamily: F.ui, fontSize: 17, fontWeight: 700 }}>כניסה עם סיסמה</div>
            </div>
            <label style={{ color: P.inkSoft, fontFamily: F.ui, fontSize: 12, display: "block", marginBottom: 6 }}>מייל</label>
            <input type="email" value={email} dir="ltr" placeholder="you@example.com" autoComplete="username"
              onChange={e => { setEmail(e.target.value); setErr(""); }}
              style={{ ...inputBase, border: `1px solid ${P.borderStrong}`, marginBottom: 12 }} />
            <label style={{ color: P.inkSoft, fontFamily: F.ui, fontSize: 12, display: "block", marginBottom: 6 }}>סיסמה</label>
            <input type="password" value={password} dir="ltr" placeholder="••••••••" autoComplete="current-password"
              onChange={e => { setPassword(e.target.value); setErr(""); }}
              style={{ ...inputBase, border: `1px solid ${err ? danger.border : P.borderStrong}` }} />
            {err && <div style={{ color: danger.color, fontSize: 12, marginTop: 6, fontFamily: F.ui }}>{err}</div>}
            <button type="submit" disabled={busy} style={buttonStyle(submitTone, { marginTop: 14 })}>
              {busy ? "מתחבר…" : "התחבר ←"}
            </button>
            <div style={{ textAlign: "center" }}>
          ...
          <>
            <button onClick={() => signInWithGoogle()} style={buttonStyle(secondary)}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 12, margin: "20px 0", color: P.inkSoft, fontSize: 12, fontFamily: F.ui }}>
              <div style={{ flex: 1, height: 1, background: P.border }} /> או <div style={{ flex: 1, height: 1, background: P.border }} />
            </div>

            <form onSubmit={sendCode}>
              <label style={{ color: P.inkSoft, fontFamily: F.ui, fontSize: 12, display: "block", marginBottom: 6 }}>כניסה עם קוד למייל</label>
              <input
              ...
              />
              {err && <div style={{ color: danger.color, fontSize: 12, marginTop: 6, fontFamily: F.ui }}>{err}</div>}
              <button type="submit" disabled={busy} style={buttonStyle(secondaryTone, { marginTop: 12 })}>
...
// onDark=true → chrome קבוע-כהה; זו החרגה מפורשת בבעלות chrome, לא palette מקומית חדשה.
export function Avatar({ profile, user, size = 36, onDark = false }) {
  ...
  const txt = onDark ? C.goldBright : P.ink;
  if (url && !imgErr) {
  ...
  return (
    <div style={{
      ...
      color: txt, fontFamily: F.ui, fontWeight: 700, fontSize: size * 0.42,
    }}>{initial}</div>
  );
}
