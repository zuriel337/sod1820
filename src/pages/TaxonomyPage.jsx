...
import { F, calcGem, isWarmNumber } from "../theme.js";
...
function TaxonomyView({ kind }) {
  ...
  return (
    <div style={{ direction: "rtl", maxWidth: 1280, margin: "0 auto", padding: "48px 18px 90px", position: "relative", zIndex: 1 }}>
      ...
      {error && <div style={{ textAlign: "center", color: "#a01f2e", fontFamily: F.body, padding: 20 }}>{error}</div>}

      {posts.length === 0 && loading ? (
        <div style={{ textAlign: "center", color: P.inkSoft, fontFamily: F.body, padding: 40 }}>טוען פוסטים…</div>
      ) : posts.length === 0 ? (
        <div style={{ textAlign: "center", color: P.inkSoft, fontFamily: F.body, padding: 40 }}>אין פוסטים {isTag ? "בתגית" : "בקטגוריה"} זו עדיין.</div>
      ) : (
        <>
          ...
          {hasMore && (
            <div style={{ textAlign: "center", marginTop: 36 }}>
              <button onClick={() => setPage(pp => pp + 1)} disabled={loading} style={{
                cursor: loading ? "default" : "pointer", background: P.cardGrad,
                border: `1px solid ${P.borderStrong}`, borderRadius: 999, color: P.accentText,
      ...
      <div style={{ textAlign: "center", marginTop: 40 }}>
        <Link to="/post" style={{ color: P.accentDim, fontFamily: F.heading, fontSize: 13, textDecoration: "none", letterSpacing: 2 }}>
          אל כל הפוסטים →
        </Link>
      </div>
      ...
      <style>{`
        .tax-card { display:flex; flex-direction:column; text-decoration:none; overflow:hidden;
          border:1px solid ${P.border}; border-radius:14px; background:${P.cardGrad};
          transition:border-color .18s, transform .18s, box-shadow .18s; animation:tax-in .5s ease both; }
        .tax-card:hover { border-color:${P.accent}; transform:translateY(-3px);
        ...
        .tax-thumb-mark { color:${P.accentDim}; font-size:30px; opacity:.5; }
        ...
        .tax-name { color:${P.accentText}; font-family:${F.regal}; font-size:17.5px; font-weight:700; line-height:1.4;
          display:-webkit-box; -webkit-line-clamp:2; -webkit-box-orient:vertical; overflow:hidden; }
        .tax-excerpt { color:${P.inkSoft}; font-family:${F.body}; font-size:14.5px; line-height:1.75;
          display:-webkit-box; -webkit-line-clamp:2; -webkit-box-orient:vertical; overflow:hidden; }
        .tax-meta { display:flex; justify-content:space-between; align-items:center; margin-top:auto; padding-top:6px;
          color:${P.accentDim}; font-family:${F.heading}; font-size:12.5px; }
