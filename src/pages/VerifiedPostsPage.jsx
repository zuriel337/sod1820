  ...
  return (
    <div style={{ direction: "rtl", maxWidth: 1080, margin: "0 auto", padding: "48px 18px 90px", position: "relative", zIndex: 1 }}>
      <div style={{ textAlign: "center", marginBottom: 26 }}>
        ...
        <h1 style={{ color: C.goldBright, fontFamily: F.regal, fontSize: "clamp(26px,5vw,42px)", fontWeight: 700, margin: 0, textShadow: "0 0 40px rgba(212,175,55,0.3)" }}>
          ✓ פוסטים מאומתים
        </h1>
        <p style={{ color: C.muted, fontFamily: F.body, fontSize: 14.5, lineHeight: 1.8, maxWidth: 480, margin: "12px auto 0" }}>
          פוסטים שהנתונים בהם (תאריכים ומספרים) נסרקו ואומתו על ידי בינה מלאכותית.
        </p>
      </div>

      {posts === null ? (
        <div style={{ textAlign: "center", color: C.muted, fontFamily: F.body, padding: 40 }}>טוען…</div>
      ) : posts.length === 0 ? (
        <div style={{ textAlign: "center", color: C.muted, fontFamily: F.body, padding: 40 }}>עדיין אין פוסטים מאומתים.</div>
      ) : (
          ...
          {posts.map(p => {
            ...
            return (
              <Link key={p.wp_id} to={`/${p.slug}`} style={{ display: "flex", flexDirection: "column", textDecoration: "none", overflow: "hidden", border: `1px solid ${C.borderGold}`, borderRadius: 14, background: "linear-gradient(160deg, rgba(20,15,12,0.55), rgba(8,5,2,0.45))" }}>
                <div style={{ position: "relative", aspectRatio: "16/10", background: img ? `center/cover no-repeat url(${img})` : `linear-gradient(135deg, ${C.goldDeep}, ${C.faint})` }}>
                  <span style={{ position: "absolute", top: 8, insetInlineStart: 8 }}><VerifiedBadge variant="ai" size={14} label="מאומת" /></span>
                </div>
                <div style={{ padding: "12px 14px" }}>
                  <div style={{ color: C.goldBright, fontFamily: F.regal, fontSize: 16, fontWeight: 700, lineHeight: 1.4, display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical", overflow: "hidden" }}>{title}</div>
                  <div style={{ color: C.goldDim, fontFamily: F.heading, fontSize: 11.5, marginTop: 6 }}>עודכן · {timeAgoHe(p.modified)}</div>
                </div>
              </Link>
            );
          })}
