...
import MatrixStack3D from "../components/els/MatrixStack3D.jsx";
...
export default function ElsWorkAreaPage() {
  ...
  // 🪟 Matrix Stack Peek v1 — projection-only. This deliberately does NOT redefine matrix truth,
  //    Finding identity or ELS geometry. The preview slices the current canonical Matrix Snapshot
  //    into sequential row-windows; a future virtualized corpus-window adapter can supply true
  //    adjacent corpus planes through the same MatrixStack3D `planes` contract.
  const [matrixStackPeek, setMatrixStackPeek] = useState(false);
  const [stackPlaneRows, setStackPlaneRows] = useState(12);
  ...
  const resetView = () => {
    ...
    setTiltX(54); setTiltZ(-7); setDepth(24); setMatrixStackPeek(false); setStackPlaneRows(12);
  ...
  return <div dir="rtl" style={{ position: "relative", zIndex: 1, maxWidth: 1700, margin: "0 auto", padding: "14px 12px 90px" }}>
    ...
    <div className="els-native-layout"><main style={{ minWidth: 0 }}><section style={{ ...card, overflow: "hidden", minHeight: 560 }}>
      <div style={{ padding: "13px 14px", borderBottom: `1px solid ${P.border}`, display: "grid", gap: 10 }}>
        <div style={{ display: "flex", justifyContent: "space-between", gap: 10, alignItems: "center", flexWrap: "wrap" }}><div><div style={{ ...title, fontSize: 14 }}>מטריצת המחקר · {viewMode === "2d" ? "מישור" : viewMode === "layers" ? "שכבות עומק" : matrixStackPeek ? "Matrix Stack 3D · Peek" : "מרחב 3D"}</div><div style={{ ...muted, fontSize: 11.5 }}>אותו Finding · אותו Snapshot · Renderer שונה בלבד</div></div>{ok && <span style={{ color: P.accentText, fontFamily: F.heading, fontWeight: 900, fontSize: 12 }}>{n((occ.index ?? 0) + 1)} / {n(occ.count)} · S={n(geo.S)}</span>}</div>
        <div className="els-view-modes">{VIEW_MODES.map((v) => <button key={v.id} onClick={() => { setViewMode(v.id); if (v.id !== "3d") setMatrixStackPeek(false); }} type="button" style={{ minHeight: 46, padding: "0 18px", borderRadius: 13, border: `1px solid ${P.border}`, background: viewMode === v.id && !(v.id === "3d" && matrixStackPeek) ? P.accentBtn : P.cardSoft, color: viewMode === v.id && !(v.id === "3d" && matrixStackPeek) ? P.onAccent : P.ink, fontFamily: F.heading, fontWeight: 900, fontSize: 13 }}>{v.icon} {v.title}</button>)}
          <button type="button" onClick={() => { setViewMode("3d"); setMatrixStackPeek((v) => !v); }} style={{ minHeight: 46, padding: "0 18px", borderRadius: 13, border: `1px solid ${P.border}`, background: matrixStackPeek ? P.accentBtn : P.cardSoft, color: matrixStackPeek ? P.onAccent : P.ink, fontFamily: F.heading, fontWeight: 900, fontSize: 13 }}>🪟 Matrix Stack{matrixStackPeek ? " · פעיל" : " · הצצה"}</button>
        ...
        <div className="els-native-toolbar" style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
          ...
          {matrixStackPeek && <label style={{ ...muted, fontSize: 11 }}>גובה מישור <input type="range" min="6" max="30" step="2" value={stackPlaneRows} onChange={(e) => setStackPlaneRows(Number(e.target.value))} /> {stackPlaneRows} שורות</label>}
          {!matrixStackPeek && <button type="button" onClick={() => setPanMode((v) => !v)} title="גרירה מזיזה את הבמה (פאן) במקום לסובב" style={{ minHeight: 42, borderRadius: 12, padding: "0 13px", border: `1px solid ${P.border}`, background: panMode ? P.accentBtn : "transparent", color: panMode ? P.onAccent : P.ink }}>✋ הזזה</button>}
          {!matrixStackPeek && sceneScale !== 1 && <span style={{ ...muted, fontSize: 11 }}>זום {Math.round(sceneScale * 100)}%</span>}
          {!matrixStackPeek && viewMode !== "2d" && <><label style={{ ...muted, fontSize: 11 }}>עומק <input type="range" min="8" max="54" value={depth} onChange={(e) => setDepth(Number(e.target.value))} /></label><label style={{ ...muted, fontSize: 11 }}>התפוצצות <input type="range" min="1" max="3" step="0.25" value={explode} onChange={(e) => setExplode(Number(e.target.value))} /></label>{viewMode === "3d" && <><label style={{ ...muted, fontSize: 11 }}>X <input type="range" min="18" max="72" value={tiltX} onChange={(e) => setTiltX(Number(e.target.value))} /></label><label style={{ ...muted, fontSize: 11 }}>Z <input type="range" min="-24" max="24" value={tiltZ} onChange={(e) => setTiltZ(Number(e.target.value))} /></label></>}</>}
      ...
      {s?.status === "empty" ? <div style={{ minHeight: 440, display: "grid", placeItems: "center", ...muted }}>לא נמצא דילוג למונח «{s.termRaw || seed}».</div> : matrixStackPeek && ok && matrix?.rows?.length ? <div style={{ padding: 12, background: P.cardSoft }}><MatrixStack3D rows={matrix.rows} cellSize={Math.max(18, cellSize - 6)} rowsPerPlane={stackPlaneRows} initialPlaneCount={3} ink={P.ink} surface={P.cardSoft} border={P.border} accent={P.accentText} /></div> : <MatrixStage />}
      {ok && matrix?.rows?.length > 0 && <div style={{ padding: "10px 14px", borderTop: `1px solid ${P.border}`, display: "flex", gap: 13, flexWrap: "wrap" }}><span style={{ ...muted, fontSize: 11 }}>● ציר ראשי</span><span style={{ ...muted, fontSize: 11 }}>◉ מסגרת = start</span><span style={{ ...muted, fontSize: 11 }}>✦ {markedCount} marks</span><span style={{ ...muted, fontSize: 11 }}>Renderer: {matrixStackPeek ? "3d-matrix-stack-peek" : viewMode}</span><span style={{ ...muted, fontSize: 11 }}>lenses: {lenses.length ? lenses.join(" · ") : "cells · marks"}</span>{verseOn && !matrixStackPeek && <span style={{ ...muted, fontSize: 11 }}>📜 Verse layer: {viewMode === "2d" ? "גבולות עדינים על התאים" : "מישור־context מתחת ל-Finding"}</span>}{matrixStackPeek && <span style={{ ...muted, fontSize: 11 }}>🪟 Preview בלבד · חלונות רציפים מאותו Matrix Snapshot · ללא Store/מנוע חדש</span>}</div>}
      {verseOn && !matrixStackPeek && <div style={{ padding: "10px 14px", borderTop: `1px solid ${P.border}` }}>
    ...
    {mode === "investigate" && <section style={{ ...card, padding: 14, marginTop: 12 }}><div style={title}>🧬 Finding Workspace</div>{!ok ? <div style={{ ...muted, marginTop: 8 }}>בחר Finding באמצעות חיפוש.</div> : <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(170px,1fr))", gap: 8, marginTop: 10 }}><div style={{ ...soft, padding: 11 }}><div style={title}>זהות</div><div style={{ ...muted, fontSize: 12 }}>{s.term} · {dir(axis.direction)} · {n(axis.skip)}</div></div><div style={{ ...soft, padding: 11 }}><div style={title}>חלון</div><div style={{ ...muted, fontSize: 12 }}>rows {n(geo.r0)}–{n(geo.r1)} · cw {n(geo.cw)}</div></div><div style={{ ...soft, padding: 11 }}><div style={title}>Renderer</div><div style={{ ...muted, fontSize: 12 }}>{matrixStackPeek ? `Matrix Stack · ${stackPlaneRows} rows/plane` : `${viewMode} · depth ${depth}`}</div></div></div>}<div style={{ marginTop: 9 }}><Next>Matrix Stack הוא Preview-Projection. השלב העתידי הוא corpus-window virtualization שמזין מישורים סמוכים אמיתיים — בלי לשנות Finding/ELS truth.</Next></div></section>}
    {mode === "judge" && <section style={{ ...card, padding: 14, marginTop: 12 }}><div style={title}>⚖️ Evidence Pack</div><div style={{ display: "grid", gap: 7, marginTop: 9 }}><div style={{ ...soft, padding: 10, color: P.ink, fontSize: 12.5 }}><b style={{ color: P.accentText }}>FACT · </b>{ok ? `«${s.termRaw || s.term}» נמצא בדילוג ${n(axis.skip)}, ${dir(axis.direction)}, start ${n(axis.start)}.` : "אין Finding פעיל."}</div><div style={{ ...soft, padding: 10, color: P.inkSoft, fontSize: 12.5 }}><b style={{ color: P.accentDim }}>DISPLAY · </b>2D/שכבות/3D/Matrix Stack הם projections של אותה אמת בלבד.</div><div style={{ ...soft, padding: 10, color: P.inkSoft, fontSize: 12.5 }}><b style={{ color: P.accentDim }}>HUMAN-GATE · </b>Canonical נשאר צוריאל בלבד.</div></div></section>}
    </main>
