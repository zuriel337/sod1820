...
export default function ElsWorkAreaPage() {
  ...
  const MatrixStage = () => {
    if (!ok) {
      return <div style={{ minHeight: 420, display: "grid", placeItems: "center", textAlign: "center", padding: 24 }}>
        <div>
          ...
          {firstRunHint && <button type="button" onClick={() => setShowEngine(true)} style={{ marginTop: 14, minHeight: 42, borderRadius: 999, padding: "0 14px", border: `1px solid ${P.border}`, background: P.cardSoft, color: P.accentText, fontFamily: F.heading, fontWeight: 850, cursor: "pointer" }}>כניסה ראשונה? פתח הדרכה חד־פעמית</button>}
        </div>
      </div>;
    }
    ...
    return <div className="els-native-stage" style={{ overflow: "auto", padding: 14, background: P.cardSoft }}>
      <div style={{ width: "max-content", minWidth: "100%", direction: "ltr", fontFamily: F.regal }}>
        {matrix.rows.map((row, ri) => (
          <div key={ri} style={{ display: "flex", justifyContent: "center", lineHeight: 1 }}>
            {Array.from(row).map((letter, ci) => {
              ...
              return <button
                ...
                style={{
                  width: cellSize, height: cellSize + 2, flex: `0 0 ${cellSize}px`, padding: 0,
                  display: "inline-grid", placeItems: "center", borderRadius: 5,
                  border: isStart ? `2px solid ${P.accentText}` : active ? `1px solid ${P.accentText}` : "1px solid transparent",
                  ...
                  fontSize: Math.max(13, Math.round(cellSize * 0.67)), opacity: dim ? 0.22 : 1,
  ...
  return (
    <div dir="rtl" style={{ position: "relative", zIndex: 1, maxWidth: 1580, margin: "0 auto", padding: "14px 12px 90px" }}>
      <style>{`
        .els-native-layout{display:grid;grid-template-columns:minmax(0,1fr) 360px;gap:14px;align-items:start}
        .els-native-inspector{position:sticky;top:12px;max-height:calc(100vh - 24px);overflow:auto}
        .els-native-modes{display:flex;gap:7px;flex-wrap:wrap}
        .els-native-stage button:hover{transform:scale(1.12);z-index:2}
        ...
        @media(max-width:560px){.els-search-form{grid-template-columns:1fr!important}.els-native-modes button{flex:1 1 auto}.els-native-toolbar{gap:5px!important}}
      `}</style>
      ...
      <header style={{ display: "flex", gap: 12, alignItems: "center", flexWrap: "wrap", marginBottom: 12 }}>
        <div>
          ...
          <h1 style={{ margin: "2px 0 0", color: P.accentText, fontFamily: F.regal, fontSize: "clamp(25px,4vw,40px)", lineHeight: 1.05 }}>הצופן התנ״כי · Research Studio</h1>
        </div>
      ...
      <form className="els-search-form" onSubmit={runSearch} style={{ ...card, padding: 10, display: "grid", gridTemplateColumns: "minmax(0,1fr) auto", gap: 8, marginBottom: 10 }}>
        <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="מה לחפש בתורה?" aria-label="מונח לחיפוש ELS" style={{ minHeight: 50, borderRadius: 12, border: `1px solid ${P.border}`, background: P.cardSoft, color: P.ink, padding: "0 14px", fontFamily: F.body, fontSize: 17, outline: "none" }} />
        <button type="submit" disabled={query.trim().length < 2 || waiting} style={{ minHeight: 50, minWidth: 118, borderRadius: 12, border: 0, background: P.accentBtn, color: P.onAccent, fontFamily: F.heading, fontWeight: 900, cursor: query.trim().length >= 2 && !waiting ? "pointer" : "default", opacity: query.trim().length >= 2 && !waiting ? 1 : .55 }}>{waiting ? "מחפש…" : "חפש ✦"}</button>
      </form>
      ...
      <div className="els-native-modes" style={{ marginBottom: 10 }}>
        ...
        <button onClick={() => setShowEngine((v) => !v)} type="button" style={{ marginInlineStart: "auto", minHeight: 42, padding: "0 12px", borderRadius: 999, border: `1px solid ${P.border}`, background: "transparent", color: P.inkSoft, fontFamily: F.heading, fontSize: 11.5, cursor: "pointer" }}>{showEngine ? "הסתר מנוע" : "Debug / onboarding"}</button>
      </div>
      ...
      <div className="els-native-layout">
        <main style={{ minWidth: 0 }}>
          <section style={{ ...card, overflow: "hidden", minHeight: 500 }}>
            <div style={{ padding: "11px 13px", borderBottom: `1px solid ${P.border}`, display: "flex", justifyContent: "space-between", gap: 9, alignItems: "center", flexWrap: "wrap" }}>
              <div><div style={title}>מטריצת המחקר · Native Renderer</div><div style={{ ...muted, fontSize: 11.5, marginTop: 2 }}>האותיות והסימונים מגיעים ישירות מ־Matrix Snapshot</div></div>
              <div className="els-native-toolbar" style={{ display: "flex", gap: 7, alignItems: "center", flexWrap: "wrap" }}>
                {ok && <span style={{ color: P.accentText, fontFamily: F.heading, fontWeight: 900, fontSize: 11.5 }}>{n((occ.index ?? 0) + 1)} / {n(occ.count)} · S={n(geo.S)}</span>}
                <button type="button" onClick={() => setFocusMarks((v) => !v)} style={{ minHeight: 34, borderRadius: 999, padding: "0 10px", border: `1px solid ${P.border}`, background: focusMarks ? P.accentBtn : "transparent", color: focusMarks ? P.onAccent : P.inkSoft, cursor: "pointer", fontFamily: F.heading, fontSize: 11 }}>◎ מיקוד</button>
                <button type="button" onClick={() => setCellSize((v) => Math.max(18, v - 3))} style={{ minWidth: 34, minHeight: 34, borderRadius: 999, border: `1px solid ${P.border}`, background: "transparent", color: P.ink, cursor: "pointer" }}>−</button>
                <button type="button" onClick={() => setCellSize((v) => Math.min(38, v + 3))} style={{ minWidth: 34, minHeight: 34, borderRadius: 999, border: `1px solid ${P.border}`, background: "transparent", color: P.ink, cursor: "pointer" }}>＋</button>
              </div>
            </div>
            ...
            {ok && matrix?.rows?.length > 0 && <div style={{ padding: "9px 13px", borderTop: `1px solid ${P.border}`, display: "flex", gap: 11, flexWrap: "wrap", alignItems: "center" }}>
