...
// Public label authority is canonical_methods_registry_law v6 §21/§25 — no local קדמי/משולש
// override here; both call shapes delegate to the one shared canonicalMethodPublicLabel projection.
function methodLabel(group) {
  return canonicalMethodPublicLabel({
    method_key: methodKey(group),
    display_label: clean(group?.registry?.display_label || group?.display_label || group?.method || group?.method_key),
  });
}

function methodProfileLabel(profile) {
  return canonicalMethodPublicLabel({ method_key: profile?.methodKey, display_label: profile?.displayLabel });
}
