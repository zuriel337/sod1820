...
import { allowAiMessage, markAiMessage } from "../lib/journeyGuard.js"; // 🛡️ מגן-קרדיט: מסע אחד = מסר-AI אחד לסשן
...
export default function JourneyPage() {
  ...
  async function fetchAiMessage() {
    ...
    // 🛡️ מגן-קרדיט: מסע אחד = מסר-AI אחד לסשן. קריאה כפולה/חריגה נחסמת ונרשמת ללוג
    // (journey_ai_blocked/journey_ai_capped) — כדי שרגרסיה לא תשרוף קרדיט בשקט.
    if (!allowAiMessage(root)) { setAiState("off"); return; }
    markAiMessage(root);
    setAiState("busy");
    ...
    <div style={{ direction: "rtl", maxWidth: 760, margin: "0 auto", padding: "34px 18px 90px", position: "relative", zIndex: 1 }}>
      ...
      {loading ? (
        ...
        <div style={{ textAlign: "center" }}>
          ...
          {/* 💬 וו-ההמרה — רמז אישי בוואטסאפ (סוגר את דליפת-המסע: מהצפייה החד-פעמית → קשר מתמשך עם הבוט) */}
          {root != null && (
            <a href={`https://wa.me/972557049261?text=${encodeURIComponent(`שלום רזיאל 🌳 הגעתי מהמסע — נחתי על ${root}${KEY_NUMBERS[root] ? ` (${KEY_NUMBERS[root]})` : ""}.\nאשמח לרמז אישי על: `)}`}
              target="_blank" rel="noopener noreferrer" onClick={() => logView("journey_wa_cta", String(root))}
              style={{ display: "block", maxWidth: 520, margin: "0 auto 16px", textDecoration: "none",
                background: "linear-gradient(135deg, #1fa855, #128c3e)", color: "#fff", borderRadius: 18,
                padding: "16px 20px", boxShadow: "0 10px 34px rgba(31,168,85,.4)", textAlign: "center" }}>
              <div style={{ display: "inline-flex", alignItems: "center", gap: 9, fontFamily: F.heading, fontSize: 17, fontWeight: 800 }}>
                ...
                רוצה רמז אישי על השם/התאריך שלך?
              </div>
              <div style={{ fontFamily: F.body, fontSize: 13.5, opacity: 0.95, marginTop: 4 }}>שלח לי בוואטסאפ — ואחשב לך במנוע, מאומת ✦</div>
            </a>
          )}
