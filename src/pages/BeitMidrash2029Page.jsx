...
function BeitMidrashHero({ registeredInactiveCount, focal }) {
  return (
    <section className="sod29-section sod29-bm-hero" data-experience-capability="beit-midrash-hero">
      ...
      {focal && (
        <div className="sod29-bm-hero-focal" data-experience-capability="beit-midrash-hero-focal">
          <span className="sod29-chip">{focal.kind === "convergence" ? "✨ קורה עכשיו" : "🌱 חדש במערכת"}</span>
          <span>{focal.label}</span>
          {focal.detail && <span className="sod29-muted">{focal.detail}</span>}
        </div>
      )}
    </section>
  );
...
function TraceView({ trace }) {
  ...
  const steps = trace?.projection?.dimensions?.trace?.steps;
...
function MethodLibrary({ shell }) {
  ...
  async function onOpenInHeichal() {
    if (!selectedKey || !sample.trim()) return;
    // Canonical resolve-before-navigate (same helper/shape as Heichal2029Page's own entry point):
    // the expression's real numeric root, not a hand-built phrase href, so Heichal opens focused.
    let focus = null;
    try {
      focus = await resolveExpressionFocus(sample.trim(), { preferredMethod: selectedKey });
    } catch { /* honest: fall through to the no-focus guard below */ }
    if (!focus?.href) return;
    research.setResearchContext?.({
      ...
      lens: "heichal",
      locale: "he",
      dimensions: { expressionFocusExplicit: true, focusOrigin: "beit-midrash-2029" },
    });
    navigate("/heichal");
  }
...
function BeitMidrashBody() {
  ...
  const systemNow = useMemo(
    () => (systemEvents && typeof systemEvents === "object" ? buildBeitMidrashSystemNow(systemEvents) : null),
    [systemEvents],
  );
  const focal = useMemo(() => resolveHeroFocal(systemNow), [systemNow]);

  return (
    <div className="sod29-beit-midrash">
      <BeitMidrashHero registeredInactiveCount={registeredInactiveCount} focal={focal} />
...
export default function BeitMidrash2029Page() {
  useEffect(() => {
    applySeo({
      ...
      noindex: true, // preview-flag surface only — never indexed while /2029 golden=beit-midrash is a preview
    });
  }, []);
  return (
    <Sod2029Shell
