...
> - **STATUS:** `APPLIED`(DB חי, `linswmnnkjxvweumprav`) — קוד-ה-RPC/RLS חי-בפרודקשן-DB (עצמאי-מ-git-deploy, לפי-מוסכמת-הפרויקט). המסמך (`docs/corpus-admission-foundation-v1-contract.md`) **fresh-port PR פתוח, טרם-ממוזג** (מחליף/משלים את PR #198 שלא-ניתן-היה-ל-raw-merge). **0 שורות-כפולות בכל-בדיקות-הגילוי-החוזר, 0 `admission_status` חדש, 7/7 `created_by`-coverage (מתוקן, סבב-4), 10/10 נתיבים דרך-השער (מאומת-חי סבב-4).** **Deploy/Live-verification — לא-רלוונטי** (שינויי-DB בלבד, אפס-שינוי-קוד-קליינט מלבד המסמך).
