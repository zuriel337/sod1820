...
15. **PREMIUM** — Access Gate, **לא** מאגר-ידע נפרד: `ONE TREE ├─ Public ├─ Premium └─ Research/Admin`. אפשר לשנות בעתיד אילו-שכבות סגורות בלי לשנות את מודל-הידע. **לא מתוכנן/מתומחר כאן** (נשאר עקרון-מבני בלבד, עקבי עם `platform_tiers_law`). **עדכון 22.8 — כבר-מאופיין (לא-עוד "טרם"):** `audits/research_dna_v1_foundation_contract/RESEARCH_DNA_V1_FOUNDATION_CONTRACT.md` §4 (LEGACY BASELINE · ADDITIVE ACCESS · PREMIUM DEPTH LAW, PR #166, Human-Gate ZURIEL, מוזג-ל-main) הוא כעת המקור-הקנוני-היחיד לאפיון-Premium המפורט — **לא-משוכפל כאן במכוון** (כדי-שלא-ליצור שני-נוסחים-סותרים). עיקרון-הליבה (נוסח-מתוקן, ratified): "Premium controls access, depth and tooling — never mathematical truth or canonical status."
