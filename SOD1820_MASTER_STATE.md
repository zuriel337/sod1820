...
---
*בסיס-עובד v2, מעודכן 25.8.2026 (§19 A/B, §21, §22). נשמר בענף `claude/raziel-capabilities-audit-h5k9ww` (מקור γ) + נכתב לגוף-המסמך על ענף `claude/els-function-inventory-86klre` + `claude/relation-engine-v1` + `claude/live-sync-v5.3-reconciliation` (סשנים). שינויי-DB בסשן v5.3 זה: הוספת `nodes.rule_id='live_state_sync_law'` בלבד (חוק-תפעולי, לא קנון-תוכן). שינויי-DB היסטוריים (סשנים קודמים): (1) הקפאת cron job 27 (`metatron-nightly`, הפיך); (2) **H-1** — RPC `fn_persist_discovery` + מועמד-בדיקה-אחד (878=משיח↔דבר-מתוך-דבר, status=`candidate`, ממתין ל-Human-Gate). מלבדם READ-ONLY. שום `INFERRED` אינו עובדה; שום שיטה לא-הופעלה; שום convergence היסטורי לא-חובר/קודם/נמחק; שום קנון לא-שונה (מלבד §0 governance + §8 FREEZE + §10 חזון + H-1 front-half + §19 A/B + §21 + §22 התיעוד-הזה).*
