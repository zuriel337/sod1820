...
### §19-E. LEGACY ACCESS PRESERVATION + PREMIUM DEPTH LAW (אפיון סעיף 15 של §19-B) — `APPROVED` (Human-Gate ZURIEL · 22.8.2026 · **DOCUMENTATION-ONLY** — אפס build/schema/migration/deploy)
> **מילוי-פער שהושאר במפורש פתוח:** סעיף 15 (§19-B, PREMIUM) קבע ש-Premium הוא Access-Gate ולא-מאגר-ידע-נפרד, אך ציין במפורש **"לא מתוכנן/מתומחר כאן... Premium טרם-מאופיין לפרטים"**. הרשומה הזו מאפיינת את העיקרון (לא-מתכננת UI/תמחור/RLS) — לא-פותחת/סוגרת Gate, לא-משנה `ACTIVE_NOW`, לא-נוגעת ב-DB/schema/קוד.

- **חוק-א — Legacy Access Preservation (שימור-גישה על יכולות-קיימות):** כל **world / method / DNA-capability / convergence / יכולת-דף-מספר** שכבר-**זמינים היום** למשתמשים במערכת החיה — נשארים ברמת-הגישה הקיימת שלהם. **Research DNA v1 אינו-משמש רטרואקטיבית לנעילת יכולות-קיימות.** המעבר לארכיטקטורת-Research-DNA (§19-A/γ + Research-DNA-v1, שלב 3 ב-`STRATEGIC FOUNDATION ORDER` שב-Roadmap) אינו-אמתלה לצמצם גישה שכבר-פתוחה.
- **חוק-ב — Premium Research Depth (facets חדשים בלבד):** worlds/facets **חדשים** שנוצרים מתוך Research DNA (למשל מימדי emotional/semantic/research-depth) **יכולים** להיות מסווגים כ-**Premium Research Depth**. זה חל **רק על facets-חדשים** — לא-רטרואקטיבית על מה-שכבר-פתוח (חוק-א נשאר-עליון).
- **חוק-ג — שיטות-חדשות = מנוע-אחד; עומק-המחקר-בלבד ניתן-להגבלה:** שיטת-גימטריה חדשה שאושרה נכנסת **למנוע-האחד הקיים** (`gematria.js`/`METHODS`) לפי `els_single_engine_law`/`gematria_engine_law` — **אין מנוע-שני**. **עצם האמת-המתמטית (הערך המחושב) אינה "Premium truth"** — אך **העומק והכלים שמעליה** ניתנים-להגבלה ל-Premium: multi-method exploration · matrices · comparison · advanced filtering · Cross Engine (`WS-CROSS-ENGINE`) · graph paths · advanced provenance · Raziel research.
- **העיקרון-המסכם (כלשונו):** **"Premium controls depth and tooling — not truth."** — מרחיב-במדויק את §19-B#15 ואת `platform_tiers_law` הקיימים; לא-סותר, ממלא-פער-שכבר-מוגדר-כפתוח.
- **תשתית-תומכת קיימת (למה זה מילוי-פער ולא-בנייה-חדשה):** §19-B#15 (Access-Gate-not-a-separate-tree, כבר-`APPROVED`) · `platform_tiers_law` (6-רמות-גישה, `CLAUDE.md`) · `WS-CROSS-ENGINE` (Cross Engine/graph-paths, `DESIGN`/`FUTURE`, Roadmap 22.8) · Research-DNA-v1 (שלב-3, `OPEN-HUMAN-GATE`, `work_log` 22.8). **אפס תשתית-חדשה נדרשת.**
- **מה-לא-הוכרע-כאן (נשאר-פתוח, לא-מומצא):** אילו facets-קונקרטיים ייכנסו-בפועל תחת Premium-Research-Depth · תמחור/RLS-בפועל · מימוש-Schema (`research_objects`/`nodes.metadata`/אחר). זו-אפיון-עקרוני-בלבד — **לא** Build-spec ו-**לא** Pricing-spec.
...
| 31 | 22.8.2026 | **§19-E LEGACY ACCESS PRESERVATION + PREMIUM DEPTH LAW (אפיון סעיף 15):** חוק-א שימור-גישה על יכולות-קיימות (Research DNA v1 אינו-נועל רטרואקטיבית) · חוק-ב Premium Research Depth חל-רק-על facets-חדשים (emotional/semantic/research-depth) · חוק-ג שיטות-חדשות=מנוע-אחד, האמת-המתמטית≠Premium אך עומק/כלים (multi-method·matrices·comparison·filtering·Cross-Engine·graph-paths·provenance·Raziel-research) ניתנים-להגבלה. "Premium controls depth and tooling — not truth" | צוריאל מילא-במפורש את הפער שסעיף 15 השאיר-פתוח ("Premium טרם-מאופיין לפרטים") | אינו-מחליף כלום; ממלא-פער-שכבר-מוגדר | `APPROVED` (documentation-only) · אפס DB/schema/build/migration/deploy · תמחור/RLS/facets-קונקרטיים נשארים-פתוחים |

---
*בסיס-עובד v2, מעודכן 22.8.2026 (§19 A/B/E). נשמר בענף `claude/raziel-capabilities-audit-h5k9ww` (מקור γ) + נכתב לגוף-המסמך על ענף `claude/els-function-inventory-86klre`, ועודכן ב-§19-E (LEGACY ACCESS PRESERVATION + PREMIUM DEPTH LAW) על ענף נפרד ב-22.8.2026. שינויי-DB בסשן-§19-E: **אפס** (documentation-only). שינויי-DB היסטוריים (סשנים קודמים): (1) הקפאת cron job 27 (`metatron-nightly`, הפיך); (2) **H-1** — RPC `fn_persist_discovery` + מועמד-בדיקה-אחד (878=משיח↔דבר-מתוך-דבר, status=`candidate`, ממתין ל-Human-Gate). מלבדם READ-ONLY. שום `INFERRED` אינו עובדה; שום שיטה לא-הופעלה; שום convergence היסטורי לא-חובר/קודם/נמחק; שום קנון לא-שונה (מלבד §0 governance + §8 FREEZE + §10 חזון + H-1 front-half + §19 A/B/E התיעוד-הזה).*
