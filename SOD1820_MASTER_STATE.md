...
## §19. ARCHITECTURE — Finding = research_object או research_contribution (design/decisions · 19.8.2026)
> **READ-ONLY findings + החלטות-כיוון. טרם-בנייה.**
- **שתי שכבות מכוונות (לא כפילות):** `research_contributions` = תרומה **ציבורית**-מיוחסת (`anon SELECT` · פורום · reactions · `approve_contribution`→`graph_node_id`; **ELS-ציבורי כבר כאן, 27 שורות `target_type='els'`**). `research_objects` = **evidence/discovery ledger שרת-בלבד** + **R1 פרטי** (`admin_research_review`→`promoted_node_id`).
- **פיצול-לפי-פרטיות (כיוון מאושר):** ממצא/ELS **ציבורי** → contribution · **אישי** (Family/Life/Hints) → object+R1. **מסע-חיים פרטי עוקף את השכבה הציבורית** (ולכן אינו תלוי בחוב-השער-הכפול).
- **`open/closed` = ציר-העל של ממצא** (משותף לשתי הטבלאות): open=candidate/idea/hypothesis/investigating · closed=approved/canonical/validated/published. **`closed ≠ fact ≠ public`** (3 צירים נפרדים).
- **החלטות-זהות (מאושר):** `persons`/`identity_edges`=גרף-זהות קנוני · `wa_account_links`=channel-linkage (לא זהות-מתחרה) · `own_in_progress_allowed=TRUE` (רזיאל קורא own draft+candidate, אך draft/candidate≠fact) · רזיאל **מציע** לשער, לעולם לא מחליט/מקדם · Person-Identity Contract (OD-F10a) + `fn_upsert_self_profile` (SELF בלבד).
- **⚠️ חוב פתוח — Human-Gate כפול:** בפועל **שני שערים** (`approve_contribution`/`graph_node_id` מול `admin_research_review`/`promoted_node_id`) — §11.27/§12.4 דורשים איחוד; זה עדיין לא-מומש. **החלטת-כיוון OPEN:** V3 (הפניה `contribution→object`) + G2 (שער-התרומות הקיים=הקנוני; `admin_research_review`=מסנן-שרת מקדים) = מומלץ, טרם-מאושר.
- **⛔ SUPERSEDED (המלצות-Claude שהוחלפו — סימון סתירות):** **(S1)** «ELS finding = `research_objects.engine_detail`» (מיפוי-Claude מוקדם) → **הוחלף** ב«ELS-ציבורי ב-`research_contributions`; `research_objects` רק לפרטי/R1» (Session 2 + ביקורת-ארכיטקטורה). **(S2)** «להוסיף `corpus_id NULLABLE` ל-`els_records`» (audit-Claude) → **הוחלף** ב«`corpus_id` חי ב-`research_objects.engine_detail`; `els_records`=curation-מפנה, ללא עמודת-corpus».
...
| 22 | 15.8.2026 | §15 «MULTILINGUAL INPUT — חוזה-קלט קנוני» (פורמליזציה של §14.2): 6 שדות לפני מנוע רגיש-טקסט (`original_value·original_language·input_type·derived_candidates·selected_research_form·provenance`) · מקור-נשמר-ללא-שינוי · DERIVED≠Fact · מועמדים+Human-Gate · `translation≠transliteration≠shared_value≠meaning` · חל על שמות/מילים-אנגלית/ביטויים/**ELS**/תאריכים/Research-Context/Raziel. EXISTING (מאומת): `translit_suggestions`/`word_aliases`/`language_links`/`name_variants`/`xlang_calibration`/`englishGematria`+`translit`/NameLab(enWord+candidates+«תעתוק מוצהר»)/תאריך-לידה(Gregorian→`gregToHebrewSpelled`). NEEDS-WIRING: קלט-ELS(`norm()` מוחק לא-עברית) · חוזה-אחיד · Research-Context/Raziel-6-שדות | צוריאל אישר את העיקרון כתשתית (Human-Gate ZURIEL 15.8.2026) | «המרה-אוטומטית=אמת/מחליפה-מקור» → «DERIVED+provenance+מועמדים+שער» | `APPROVED` (עיקרון) · Audit READ-ONLY · אפס schema/engine/UI/ELS/NameLab · חיווט=`ROADMAP` (אישור-נפרד) · נרשם על-branch `claude/premium-research-audit-bzmjop`, סנכרון-main=בנפרד |
