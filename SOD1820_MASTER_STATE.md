...
- **`corpus_id` תנ"ך — `DECIDED`+`APPLIED` (Gate #5, Human-Gate ZURIEL · 25.8.2026, אישור בצ'אט: "כן תאשר את התנ״ך הזה"):** ה-`corpus_id` הקנוני (`0b022e8eef6f9c16`) **מעולם לא היה ספציפי לתורה** — הוא מוגדר במקור (הערת-provenance ב-`20260726_name_protocol_wave2_1_els_real.sql`) כ-`sha256(tools/els/data/tk-letters.txt הקובץ-המלא)[0:16]`, כלומר טביעת-האצבע של **כל התנ"ך המוטמע** (1,204,583 אותיות), לא של קידומת-התורה (304,805, שיש-לה hash נפרד ולא-בשימוש). אומת-מחדש שרת-לשרת בסבב הזה (sha256 חי של הקובץ על-הדיסק תואם בדיוק). התורה היא **קידומת מדויקת** של אותו קובץ-קורפוס יחיד — "תורה" ו-"תנ״ך" הם שני **תחומי-חיפוש (scope)** על-גבי **אותו קורפוס-אחד**, לא שני קורפוסים נפרדים. `fn_els_corpus_id(p_scope)` עודכן (additive) כך ש-`p_scope IN ('torah','tanakh')` שניהם מחזירים את אותו `0b022e8eef6f9c16`; scope לא-מוכר ממשיך להחזיר `NULL` (fail-safe ללא-שינוי). `save_els_matrix`/`save_els_matrix_anon` לא-נגעו — שניהם כבר קוראים ל-`fn_els_corpus_id(p_scope)` ומקבלים את התיקון אוטומטית. אומת round-trip חי (`save_els_matrix_anon` עם `p_scope='tanakh'` → `corpus_id='0b022e8eef6f9c16'` בפועל בשורה; שורת-הבדיקה נמחקה מיד, 0 נותרו). **לפי מדיניות Gate #4 הקיימת (INSERT-only, אין batch-assign)**: 43 רשומות `els_records` קיימות עם `scope='tanakh'` ו-`corpus_id=NULL` **לא עודכנו רטרואקטיבית** — נשארות תחת אותה מדיניות opt-in-re-anchor כמו שאר הרשומות הישנות; רק שמירות **חדשות** מקבלות מעכשיו corpus_id נכון. Gate #5 (Canonical Gate Map, שורה 5) `CLOSED`. Migration: `20260825104425_els_tanakh_corpus_id_gate5.sql`.
