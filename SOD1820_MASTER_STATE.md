...
| 37 | 26.8.2026 | **`CORPUS_ADMISSION_FOUNDATION_V1` — §23.9 חדש (Foundation Frozen, PR #198 פתוח, טרם-ממוזג):** אודיט-מלא של 22 נתיבי-כתיבה חיים ל-`gematria_words` → שער-זהות-משותף-אחד (`fn_corpus_admission_gate`, שימוש-חוזר ב-`fn_resolve_word_identity`/`fn_normalize_for_calc`/`find_similar_words` הקיימים) עם 4 רמות-זהות; נסגרו bypass-ים אמיתיים (RLS-INSERT-פתוח, `add_entity` ללא-בדיקה, VIP עוקף-תור, `p_safe_to_auto` עוקף-תור); תוקן דריפט-אחד מול `CORPUS_APPROVAL_LIFECYCLE.md` הקנוני (`resolve_word_review` אישור מעלה כעת `is_verified=true`, לא-רק `visibility_reason`); הוכח `admission_status` לא-נדרש | Human-Gate ZURIEL/GPT — audit-driven Foundation-closure לפני-Writer-3, למנוע מילים-כפולות/bypass-ים-שקטים | אינו-מחליף-כלום; מרחיב `gematria_auto_registry_law`/`corpus_admission_lifecycle_law` הקיימים בשכבת-שער-לפני-כתיבה | `APPLIED`(DB חי) · PR#198 `mergeable_state=clean` **טרם-ממוזג** · 10/10 נתיבי-יצירת-מילה משתמשים-בשער (מאומת) · 0 שורות-כפולות בכל-בדיקות-רגרסיה-חיות · 0 שיטת-גימטריה-חדשה · 0 `admission_status` חדש |
...
### 23.9 CORPUS ADMISSION FOUNDATION v1 — FOUNDATION FROZEN (`APPLIED`, Human-Gate ZURIEL/GPT architectural review מאושר, 26.8.2026, PR **טרם-ממוזג**)

> **מהות:** אודיט מלא (22 נתיבים חיים) שיכולים ליצור/לקדם/לאמת שורת `gematria_words`, ואז סגירת-כל-ה-bypass-ים שנמצאו בפועל. נבנה ונבחן מול קורפוס-אמיתי (אותם קורפוסי-בדיקה) ומול החוזה הקנוני **הקיים-מראש** `audits/research_dna_v1_foundation_contract/CORPUS_APPROVAL_LIFECYCLE.md` (22.8.2026) — לא הומצא lifecycle מקביל; תוקן-דריפט אחד אמיתי מול אותו חוזה (ר' למטה).

- **תוצאה:** `FOUNDATION SUFFICIENT`. אפס-bypass נותר מבין 10 הנתיבים היוצרי-מילה (`wa_add_word`/`wa_add_vip_word`/`enqueue_word_review`/`resolve_word_review`/`admin_add_word`/`admin_add_alias`/`admin_edit_alias`/`promote_finding_to_dict`/`admin_promote_contrib_card`/`wizard_build_convergence`) — כולם עוברים כעת דרך שער-זהות משותף-אחד.
- **Extension Contract — `corpus_admission_foundation_v1`** (`nodes.rule_id`, גוף מלא ב-`project_codex.slug='corpus_admission_foundation_v1'` + `docs/corpus-admission-foundation-v1-contract.md`): **שער-זהות אחד** (`fn_corpus_admission_gate`, עוטף `fn_resolve_word_identity` הקיים — משתמש-חוזר ב-`fn_normalize_for_calc`/`find_similar_words` הקיימים, אפס-לוגיקה-מטושטשת-חדשה) · 4 רמות-זהות EXACT_MATCH/SAFE_NORMALIZED_MATCH/POSSIBLE_VARIANT(לעולם-לא-auto-merge)/NEW · **גילוי-חוזר** — כתב חדש שמגלה-מחדש מילה-קיימת-עם-ערך-נטען → שורת `research_objects` (משתמשת-חוזרת, `meta.ext.corpus_admission`), **בלי** ערך-נטען → אין-שורה (מונע טענה-מזויפת) · **סגר INSERT-RLS** על `gematria_words` (היה כל-משתמש-רשום → אדמין-בלבד) · **`add_entity` חוזק** (היה `PUBLIC`-executable בלי-בדיקה → `SECURITY DEFINER`+admin, **אומת כלגיטימי** מול `gematria_auto_registry_law` הנעול — מילה-חדשה-בלי-טענה-פרשנית מותרת-קנוניזציה-אוטומטית ממילא, השער רק מחמיר-מעבר-למינימום) · **VIP≠bypass** (`wa_add_vip_word` — מילה-חדשה נכנסת-לתור כמו-כל-מקור, לא-עוד-חריגה) · **`p_safe_to_auto`≠bypass** (superseded, מתעד-בלבד כעת) · **`created_by`** נוסף ל-7 נתיבי-אדמין-מפורשים.
- **תיקון-דריפט אמיתי (מול `CORPUS_APPROVAL_LIFECYCLE.md`):** `resolve_word_review` בענף-אישור עצר בעבר ב-`visibility_reason` בלבד; `gematria_words.is_verified` **הוא** דגל-Approved-Corpus (Stage 6) לפי החוזה הקנוני, לא-פרוקסי-לחישוב-מנוע (זה תפקיד `research_objects.engine_verified`) — תוקן: אישור/עריכה מעלים כעת `is_verified=true` (מזוהה-דרך word_id של השער, לא phrase-פשוט) **וגם** `visibility_reason`.
- **`admission_status` — הוכח-שלא-נדרש:** החוזה הקנוני כבר-ממפה כל-שלב לשדה-קיים ומצהיר-במפורש "לא-טבלה/עמודה/state-machine-חדשה"; אחרי תיקון `resolve_word_review`, `is_verified` נושא-משמעות-עקבית לכל-הנתיבים. **אפס עמודה חדשה.**
- **מה לא-נעשה כאן (מפורש):** אין שיטת-גימטריה חדשה · אין טבלה/engine/store מקביל · אין שינוי-התנהגות-VIP/וואטסאפ (המשתמש עדיין-מקבל-עדיפות/סדר, לעולם-לא-מקצר-Human-Gate) · אין UI חדש · אין Writer 3 · אין merge/deploy עדיין.
- **STATUS:** `APPLIED`(DB חי, `linswmnnkjxvweumprav`) — PR **#198** (`claude/corpus-admission-foundation-v1`→`main`, head `34ca0df8`, `mergeable_state=clean`) **פתוח, טרם-ממוזג**. רגרסיה חיה מלאה בוצעה (10/10 נתיבים משתמשים בשער, 0-שורות-כפולות בכל-בדיקות-הגילוי-החוזר, VIP/safe_to_auto לא-עוקפים, אישור/עריכה מעלים is_verified, RLS+כל-בדיקות-האדמין נשארו-תקינות). **Deploy/Live-verification — לא-רלוונטי** (שינויי-DB בלבד, אפס-שינוי-קוד-קליינט מלבד המסמך).
...
**עדכון נוסף, 26.8.2026 (§23.9, `CORPUS_ADMISSION_FOUNDATION_V1`):** נכתב-לגוף-המסמך על ענף `claude/corpus-admission-foundation-v1`. שינויי-DB (חיים על `linswmnnkjxvweumprav`, **לא** בענף): `project_codex.slug='corpus_admission_foundation_v1'` + `nodes.rule_id='corpus_admission_foundation_v1'` (חוזה-תיעודי) + מדיניות-RLS-אחת על `gematria_words` + 2 פונקציות-חדשות (`fn_resolve_word_identity`,`fn_corpus_admission_gate`) + 10 פונקציות-קיימות עודכנו (ר' §23.9 לרשימה) + רשומות `work_log` coordination. שינויי-קוד (על-הענף, **טרם-ממוזג**): `docs/corpus-admission-foundation-v1-contract.md` (חדש) בלבד — אפס-שינוי ל-`src/`. PR **#198** פתוח מול `main`, `mergeable_state=clean`, לא-ממוזג. **0 שינוי ל-`gematria_words`/`research_objects` היסטוריים, 0 שיטת-גימטריה חדשה, 0 `admission_status` חדש.***
