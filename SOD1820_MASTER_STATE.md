...
### §19-C. הבהרות מחייבות ל-§19-B (Human-Gate ZURIEL · 20.8.2026, הורחב 22.8.2026) — **אינן משנות את 18 הסעיפים עצמם**
> תוקן ממוקד, ללא audit חוזר. מבהיר/מחדד שבעה סעיפים קיימים — לא מוסיף עקרון חדש לחוזה. A-C אושרו 20.8.2026; D-G אושרו 22.8.2026 (Human-Gate ZURIEL) בעקבות Targeted Gap Check ממוקד (`work_log 99f1ae69`, בדיקת חומר-המחקר של ציון סיבוני) שמצא ש-D/E/F/G כולם ניתנים לייצוג מלא באמצעות התשתית הקיימת — **ללא Contract v2, ללא schema/table/engine/tree חדש**.
...
- **D. זמן = ממד-מחקרי לגיטימי (הרחבת סעיפים 1/9):** Gregorian date · Hebrew date · time · day-of-week · parasha · year/period · elapsed-days · event-relative time הם כולם ממדים זמניים לגיטימיים של אותו Contract. כל ערך-נגזר נשמר כטרנספורמציה משלו (`Input → Method → Output → Provenance`) בדיוק כמו כל ערך-מנוע אחר — **אין להסיק משמעות מעצם ההתכנסות הזמנית**. הסכמה הקיימת (`gallery_images.occurred_at` · `nodes.hebrew_date` · `research_objects.source_ref`/`engine_detail`) כבר מייצגת זאת **ללא schema חדש**. **הבהרה חשובה:** יכולת-חישוב calendar/elapsed-days/parasha **אינה קיימת כיום כמנוע מלא** (`fn_hebrew_date` הקיים ממיר תאריך-עברי-מאוית לגימטריה, לא ממיר-לוח) — אינה מוצגת כ-`IMPLEMENTED`. ר' `PLANNED/NOT IMPLEMENTED` ב-Roadmap (`WS-URC`/מרשם-העתיד).
- **E. Method-slot קנוני לכל טרנספורמציה, לא רק גימטריה (הרחבת סעיף 3):** המוסכמה `Object A → Method/Transformation → Object B` (`relation_evidence.method` · `research_objects.engine_detail`) קנונית לכל סוגי-הטרנספורמציות — לא גימטריה-בלבד. דוגמאות: `phrase→gematria→number` · `date→elapsed_days→number` · `Gregorian→calendar_conversion→Hebrew date`. יש לשמר `input · method · output · source · evidence · provenance · verification status`; **אסור להפוך Transformation ל-Fact מעבר למה שאומת בפועל במנוע**.
- **F. התכנסות מורכבת/רב-ממדית ≠ אמת (הרחבת סעיף 7/9):** Research Finding אחד רשאי לאגד רכיבים מסוגים-שונים (Verse·Phrase·Number·Date·Event·Parasha·Person·ELS·Image·External Source·Interpretation) באמצעות מנגנוני-ההרכבה הקיימים (`relates[]`·`parent_id`·`metadata`) וה-One Knowledge Tree — כל רכיב ממשיך לשמור type/provenance/status משלו. **Convergence ≠ Truth:** הופעת מספר/פסוק/תאריך משותף אינה הופכת את הקשר ל-Fact/Canonical/Published. **אין Finding Table חדשה.**
- **G. תמונה = Entry Context/Source Adapter, לא מערכת מקבילה (הרחבת סעיף 10/11):** קליטת-תמונה כמקור-מחקר עוברת את אותה שרשרת כמו כל הקשר-כניסה אחר: `Original Image → Source/Provenance → Visual/OCR Extraction → Extracted Claims/Objects → Deterministic Verification where possible → Cross-reference → Finding → Interpretation → Human Gate`. OCR/AI-vision/תצפית-ויזואלית = Extraction, **לא Fact**. יש לשמר את התמונה-המקורית ואת ה-provenance שלה. התשתית הקיימת (`gallery_images` · `gallery-ocr` · `wa-ocr` + אותה מוסכמת `source_ref` אל `research_objects`/`relation_evidence`) היא בסיס ה-adapter — **אין "מערכת מחקר-תמונות" מקבילה, אין table/engine/tree חדש**.
...
| 31 | 22.8.2026 | **§19-C הורחב — 4 הבהרות נוספות (D-G, ללא audit חוזר, בעקבות Targeted Gap Check `work_log 99f1ae69`):** D. זמן=ממד-מחקרי-לגיטימי (Gregorian/Hebrew-date/day-of-week/parasha/elapsed-days נשמרים כטרנספורמציה נפרדת עם provenance; **calendar-calculation engine אינו קיים כיום**, מסומן `PLANNED/NOT IMPLEMENTED`) · E. Method-slot קנוני לכל טרנספורמציה, לא-רק-גימטריה (`relation_evidence.method`/`research_objects.engine_detail`) · F. התכנסות-מורכבת≠אמת (`relates[]`/`parent_id` מרכיבים ממצא-רב-ממדי, כל רכיב שומר type/provenance/status משלו, אין Finding Table חדשה) · G. תמונה=Entry-Context/Source-Adapter (לא מערכת-מחקר-תמונות מקבילה; `gallery_images`/`gallery-ocr`/`wa-ocr`+אותה מוסכמת `source_ref` הם בסיס ה-adapter) | צוריאל: לקבע את תוצאות ה-Gap-Check כהבהרות ל-Contract הקיים, לא Contract v2 | אינו-מחליף כלום; מוסיף-בלבד ל-§19-C | `APPROVED` (Human-Gate ZURIEL) · documentation-only · אפס DB/schema/migration/build/deploy · אפס merge-ל-main |
| 30 | 20.8.2026 | **תיקון ממוקד ל-§19-B (ללא audit חוזר):** §19-C — 3 הבהרות מחייבות (A. Entry Context=חלק-מזהות-המחקר/provenance, לא רק UI · B. Method-Preserving Discovery=חובה לא-אופציה, דוגמת ELS-1258-מילוי מחייבת · C. Focused-Result≠Full-Research-Context, הפרדת Entry-surface/Research-surface מפורשת) · §19-D STATUS (18-הסעיפים נשארים מתועדים-ומאושרים; **המסגרת-הרחבה-יותר נשארת `OPEN`** ל-review-נוסף, לא `CLOSED`) · §0 סעיף 15 חדש — CONTEXT INTEGRITY / NO CONTEXT LOSS LAW (Context-Reconstruction לפני החלטה-מהותית/CLOSED; הפרדת Architecture-Decision/Contract/Feature/Example/Open-Question/Build-Task; איסור-הנחה-שהנושא-האחרון=מטרת-העל) | צוריאל: הבהרות ממוקדות לחוזה + חוק-עבודה כללי למניעת אובדן-הקשר, בלי לפתוח סבב-audit חדש | אינו-מחליף כלום; מוסיף-בלבד | `APPROVED` · documentation-only · אפס DB/build/migration/deploy · אפס merge-ל-main |
...
---
*בסיס-עובד v2, מעודכן 22.8.2026 (§19-C הורחב D-G). נשמר בענף `claude/raziel-capabilities-audit-h5k9ww` (מקור γ) + נכתב לגוף-המסמך על ענף `claude/els-function-inventory-86klre` (§19 A/B, 20.8) + `claude/raziel-control-center-u9geez` (§19-C D-G, 22.8, סשן זה — Human-Gate ZURIEL, בעקבות Targeted Gap Check `work_log 99f1ae69`). שינויי-DB בסשן זה: **אפס** (documentation-only; רשומת work_log אחת בלבד לתיעוד ה-Gap-Check, נכתבה בסבב הקודם). שינויי-DB היסטוריים (סשנים קודמים): (1) הקפאת cron job 27 (`metatron-nightly`, הפיך); (2) **H-1** — RPC `fn_persist_discovery` + מועמד-בדיקה-אחד (878=משיח↔דבר-מתוך-דבר, status=`candidate`, ממתין ל-Human-Gate). מלבדם READ-ONLY. שום `INFERRED` אינו עובדה; שום שיטה לא-הופעלה; שום convergence היסטורי לא-חובר/קודם/נמחק; שום קנון לא-שונה (מלבד §0 governance + §8 FREEZE + §10 חזון + H-1 front-half + §19 A/B/C-D-G התיעוד-הזה).*
