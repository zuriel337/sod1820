...
| 58 | 5.9.2026 | **`BOOK_DOSSIER_285_RECONCILIATION_V1` — §23.30 חדש (PROPOSED): תוכנית-חיבור Source→Screen עבור ספרי-מחקר (אהבת תורה/ספר הפליאה), role=INTEGRATION_OWNER, בתגובה לפקודת-תיאום GPT/ZURIEL (`work_log 807d439f`).** מזהה 3 מסכי-ספר מתחרים על נתיב `/book` (סטטי GPT ב-`main`, תוספת-טבלאות Claude ב-PR#331, React Book Hub GPT ב-ענף נפרד שמוחק את ה-rewrite הסטטי) ומציע מסלול-אחד מאוחד + מטריצת KEEP/ADAPT/PARK. אין מיזוג/יישום — תוכנית-בלבד, ממתינה למניפסט role=SOURCE_OWNER (טרם-נכתב) ולביקורת ZURIEL. ר' §23.30 למטה + `docs/research-library/ahavat-torah/BOOK_DOSSIER_285_INTEGRATION_COMPOSITION_PLAN_V1.md`. | GPT/ZURIEL coordination memo (`work_log 807d439f`) | לא-הוחלף שום סעיף. §23.26-§23.28 נשארים כלשונם — §23.30 מוסיף-בלבד תוכנית-חדשה | `PROPOSED`(docs-only) · `NOT MERGED` · `DEPENDENCY_PENDING` על מניפסט SOURCE_OWNER |
| 59 | 5.9.2026 | **`BOOK_ONE_TREE_APPROVED_EXECUTION_20260905` — §23.30 עודכן-אדיטיבית (Phase A): Human-Gate ZURIEL אישר-כיוון + GPT-review סיפק 6 תיקונים-מחייבים.** מסיר את `DEPENDENCY_PENDING` (מניפסט SOURCE_OWNER נמסר, `bd0c2ea3`) — תוקן: זהות-Book≠זהות-בחירת-מקור (אומת-חי `research_items UNIQUE(user_id,bucket,entity_type,entity_ref)`, metadata לא-חלק-מהמפתח — התוכנית-הקודמת הייתה-שוברת שתי-שמירות-שונות); הוסף-למחקר≠קידום-אוטומטי ל-`research_objects`; אין-מנצח-אמת בין שני ה-dataset-snapshots (GPT/Claude) — המקור נשאר קבצי-git+מניפסט; PDF-ציבורי≠רישיון-פרסום-פרטי; חוסר-enum-אחיד אינו-Foundation-blocker. נוספה מטריצת-יכולות 14-שורות (a-n). Phase B (vertical-slice מוגבל) **ממתין** לטסטים + ענף-מוצר נפרד — לא בוצע כאן. | Human-Gate ZURIEL + GPT review (`work_log 81611e63`) | לא-הוחלף שום סעיף היסטורי — §23.30 המקורי (שורה 58) נשאר-כלשונו, זו תוספת-מתקנת בלבד | `PROPOSED→PARTIALLY-APPLIED`(docs-only) · `NOT MERGED` · Phase B `NOT STARTED` |
...
## §23.30 — BOOK DOSSIER #285 RECONCILIATION — INTEGRATION COMPOSITION PLAN, PROPOSED (5.9.2026, actor=CLAUDE role=INTEGRATION_OWNER, Human-Gate ZURIEL request via GPT coordination)

> **מעמד:** pointer/PROPOSED-בלבד — **אין שכפול-גוף כאן.** הטקסט המלא חי ב-`docs/research-library/ahavat-torah/BOOK_DOSSIER_285_INTEGRATION_COMPOSITION_PLAN_V1.md`, ענף `claude/book-dossier-285-integration-plan`, **לא-ממוזג**. שום-סעיף-כאן אינו קובע ארכיטקטורה סופית — זו תוכנית-חיבור לביקורת, לא-החלטה.

**מקור:** פקודת-תיאום GPT/ZURIEL (`work_log 807d439f-085d-4956-8f63-4af21ddd3907`, task=`BOOK_DOSSIER_285_RECONCILIATION_V1`) הקצתה שני תפקידים לשני סשני-Claude: `role=SOURCE_OWNER` (הסשן המחזיק PR#285) ו-`role=INTEGRATION_OWNER` (הסשן המחזיק ענף-הטבלאות החדש, PR#331). סעיף זה הוא פלט-role=INTEGRATION_OWNER בלבד; מניפסט role=SOURCE_OWNER **טרם נכתב** ב-work_log נכון-לזמן-כתיבה — חלקים-שתלויים-בו מסומנים במפורש כ-`DEPENDENCY_PENDING` במסמך המלא.

**הממצא-המרכזי:** שלושה מסכי-ספר נפרדים תובעים כרגע את הנתיב `/book`: (1) `public/book.html` הסטטי (GPT, **כבר-חי בפרודקשן**), (2) תוספת-הטבלאות-הגנרית (Claude, PR#331, ענף `claude/ahavat-torah-closure-matrix-lf161n`, `NOT MERGED`), (3) React Book Hub מבוסס-DB (GPT, ענף `gpt/book-research-context-spatial-v1`, `NOT MERGED`) — **שמוחק בפועל** את ה-Vercel rewrite הסטטי ומחליפו ב-React Router. אם (3) ימוזג לבד, (1)+(2) הופכים לקוד-מת. התוכנית ממליצה על **מסלול-אחד מאוחד** (React Book Hub כבסיס, יכולת-הטבלאות-הגנרית מועברת-פנימה כטאב, לא-נשארת דף-סטטי-נפרד) + מטריצת KEEP/ADAPT/PARK מלאה + תוכנית-מסך בעברית + רשימת-אימות-לביצוע.

**מצב-DB חי מאומת בסבב-זה (לא-לשנות, רק-לתעד):** `entity_types.type='book'` ו-`nodes.type='book'` (2 שורות: `book:ahavat-torah`, `book:sefer-hapliah`) **כבר-חיים** — המיגרציה שבענף GPT כבר-הורצה בפועל, אין-להריץ-שוב. `research_objects` עם `source_ref` שמתחיל ב-`book:hebrewbooks:5635`/`hebrewbooks:5635`: **2 שורות בלבד**, לעומת עשרות-datasets/187-שורות-אותיות/70-content_blocks/10-סתירות שקיימות **רק ב-git** (PR#285 ואחיו) ולא-הוזנו-מעולם ל-`research_objects`.

**מה לא-נעשה בסבב-הזה:** אין scan מחדש, אין merge, אין deploy, אין schema/DB/RLS/engine write, אין canonicalization/publication, אין ingestion המוני, אין הרחבת-3D, אין עבודת-טיפוגרפיה/P1/ELS. `ACTIVE_NOW` לא-שונה.

**Provenance:** `work_log 807d439f` (GPT coordination memo) · ACK role=INTEGRATION_OWNER (`work_log`, אותו סשן) · ענפים נבדקו-חיים: `origin/main`@`bae282da`, PR#285@`004fc421`, PR#331@`a5b635f3`, `gpt/book-research-context-spatial-v1`@`8545a292` · `docs/research-library/ahavat-torah/BOOK_DOSSIER_285_INTEGRATION_COMPOSITION_PLAN_V1.md` (הגוף המלא).

### עדכון Phase A (5.9.2026, `BOOK_ONE_TREE_APPROVED_EXECUTION_20260905`, Human-Gate ZURIEL + GPT review, `work_log 81611e63`)

> **תוספת-אדיטיבית בלבד — הטקסט למעלה נשאר כלשונו.** ZURIEL אישר-כיוון ("עץ אחד מלמטה למעלה", מחקר-ספרים לא-ממתין להשלמת דף-המספר). GPT-review סיפק 6 תיקונים-מחייבים על התוכנית (`docs/.../BOOK_DOSSIER_285_INTEGRATION_COMPOSITION_PLAN_V1.md` §6): **הכי-קריטי** — זהות Book-level (`entity_ref='book:ahavat-torah'`, נשמר-פעם-אחת) הובחנה במפורש מזהות source-selection (locator ספציפי-לשורה/בלוק, למשל `book:hebrewbooks:5635#p36:row_12`) אחרי שנמצא-חי ש-`research_items` נושא `UNIQUE(user_id,bucket,entity_type,entity_ref)` בלי `metadata` במפתח — התכנון-הקודם היה-שובר שתי-שמירות-שונות-של-הספר-עצמו. `DEPENDENCY_PENDING` (מניפסט SOURCE_OWNER) **הוסר** — המניפסט נמסר (`bd0c2ea3`, `AHAVAT_TORAH_SOURCE_MANIFEST_285.md`, commit `074013d6`) ונוסף-בציטוט. נוספה מטריצת-יכולות 14-שורות (owner/מצב/תלות/פעולה-הבאה/אימות לכל יכולת). **Phase B (vertical-slice מוגבל Book→locator→Workspace) ממתין** לטסטים (שמירת-2-שורות-שונות ניצולות רענון; אידמפוטנטיות; snapshot-policy) ולענף-מוצר נפרד מ-`main` — **לא בוצע בסבב-הזה**. אין schema/DB/RLS/RPC/merge/deploy. `ACTIVE_NOW` לא-שונה.
