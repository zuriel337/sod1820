...
- **`post_text_colors_law` (חקוק):** בפוסט נקי — **טקסט רץ לבן; גימטריה זהובה ולחיצה**: כל ביטוי-גימטריה עטוף `<a href="/number/<ביטוי>">` וכל ערך `<a href="/number/<ערך>">` (בעיקר הביטוי). הסגנון קנוני ב-POST_CONTENT_CSS (`.sod-post-content.clean`) — לא לצבוע אפור-בז' ב-inline.
