...
- **⛔ בוטסטרפ-סוכן / "מה המצב התפעולי עכשיו" — אסור `select * from work_log order by created_at desc` גולמי.** שאילתה גולמית מערבבת הוראות-נוכחיות עם רשומות ישנות/מוחלפות בלי סמן-סמכות. **תמיד** לקרוא דרך ההקרנה הקנונית: `select * from public.get_work_log_current();` (או `select * from public.work_log_current;`) — מחזירה **רק** `archived=false AND superseded_by_id IS NULL`.
...
- **קריאה היסטורית/audit מפורשת** (לא לבוטסטרפ שוטף) — כל הרשומות כולל archived+superseded: `select * from public.get_work_log()` (RPC קיים, ללא שינוי; מוגבל ל-1000 האחרונות) או `select * from public.work_log_view` (חושף `archived`+`superseded_by_id` לצפייה אנושית) או `select * from work_log` ישירות כשמפורש נדרש כל ההיסטוריה.
