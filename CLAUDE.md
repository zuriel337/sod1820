...
- **קול המערכת — לשון רבים, מלמעלה (`post_system_voice_law`, נעול בליבה):** פוסטים באתר נכתבים בקול המערכת — **לשון רבים** («אנחנו בודקים», «אנחנו חושבים», «אנחנו מפענחים», «תפקידנו»), כמערכת שמדברת **מלמעלה**. **אסור** קול אישי «אני», **ואסור** פנייה של «חברים». זו החלטה מבוססת-שנים של צוריאל; עקביות הקול גוברת. חריג נקודתי (עדות אישית בפוסט מסוים) = החלטה מפורשת של צוריאל בלבד. החוק המלא: `select description from nodes where rule_id='post_system_voice_law';`.
- **ויזואל מוטמע בפוסט = inline בלבד, בלי `<style>` (`post_embedded_visual_law`, נעול בליבה):** כל ויזואל מוטמע בתוכן פוסט (מטריצת ELS, גריד, תאים צבועים, פריסה מותאמת) נבנה עם **סגנון inline על כל אלמנט** (`style="..."`). **אסור `<style>`+קלאסים מותאמים בתוך תוכן הפוסט** — מרנדר הפוסטים (`src/legacy/legacy.jsx` ~שורה 4379) **מסיר כל בלוק `<style>`** מטעמי אבטחה, ואז הקלאסים חסרי-סגנון והגריד קורס ל«ג׳יבריש»/ערימת אותיות. **חריג:** קלאסים שכבר מוגדרים גלובלית ב-`POST_CONTENT_CSS` (`sod-post-content`, `sod-gematria-box`, `gb-title`, `gb-rows`, `gb-note`) — מותרים (הם ב-stylesheet של האפליקציה, לא נמחקים). כללים: (1) תאים = `<span>` עם `display:inline-flex`, לא `<div>` עם height (מנקה שורה 4386 מוחק `div` ריק עם height). (2) חובה **לאמת ויזואלית** (Read צילום) ברוחב מובייל לפני פרסום. (3) מובייל-ראשון: תאים ≤26px שנכנסים ל-360px. החוק המלא: `select description from nodes where rule_id='post_embedded_visual_law';`.
