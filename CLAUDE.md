...
- **עדכונים אחרונים** = פוסטים (רק צוריאל מעדכן פוסטים). הרכיב `LatestPostsRail`.
  - **עדכון (HomeNewPage):** ל"עדכונים אחרונים" מתווספים גם **רמזים מזרם המציאות** (`gallery_images source='update'`) ככרטיסי-תמונה עם **רצועת-מספר ממותגת** (המספר הדומיננטי + ✦סוד 1820), ממוזגים לפי תאריך. לחיצה → דף המספר. כך "רואים שעלה עכשיו עדכון גלריה". (`updatesFeed` ב-`HomeNewPage.jsx`.)
- **חידושי AI** = תיבה מכובדת עם 3 חידושים אחרונים (`origin='ai'`) + "עוד בבית המדרש →". הרכיב `AiInsightsBox`.
- היכל השערים (`heichal.html`) — הוסר ה-overlay "סוד 1820" שהסתיר את המרכז.
...
- ✍️ **`brand_spelling_law` (חקוק — נשבר שוב ושוב):** כתיב שם-המותג. בעברית = **«סוד 1820» עם רווח** בין «סוד» ל-«1820» (לעולם לא «סוד1820» צמוד). באנגלית/לטינית = **«sod1820»/«SOD1820» בלי רווח** (וכן הדומיין `sod1820.co.il`). חל בכל טקסט מוצג/נוצר: פוסטים, כרטיסים, כתוביות וקרדיטים («תרגום ועריכה: סוד 1820»), טיקרים, פרסונות-AI, שיתופים, SEO/fallbacks. הצורה הקנונית בקוד = «✦ סוד 1820». החוק המלא: `select description from nodes where rule_id='brand_spelling_law';`.
