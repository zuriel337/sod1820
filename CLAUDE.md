...
- **אפס כפילות:** הטיקר העליון (`LiveActivityBar`) מציג עדכון-חי רק כ**מצביע** («← לצפייה») ומוסתר ב-`/` וב-`/community/chat`. תקן: ● LIVE אדום + אייקון וואטסאפ, בלי מילים. **קרדיט «מאת» חובה על כל עדכון.**
...
- `vercel.json` מכיל `ignoreCommand` שמדלג על בניות שאינן `main` — ⛔ לא להסיר (חוסך את המכסה).
