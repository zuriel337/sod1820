...
| Agent coordination / owner creation / parallel write safety / 2029 fresh-agent entry | `inter_agent_coordination_law` | versioned rule | **v9 ACTIVE · DB-LIVE 2026-09-12** | `nodes(type='rule')`; `work_log` is coordination channel; v9 extends v8 with explicit G1 Agent Entry / Bootstrap Reconciliation acceptance: owner-first, minimal dependencies, live verification, stale active-entry cleanup, adapters/memory/codex as pointers/context only, no parallel SSOT/context system, fresh-agent replay required before G1/higher-gate closure |
