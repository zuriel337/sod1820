...
## Canonical owner map — selective live re-verification 2026-09-13

Versioned rule rows represented below were re-read live from canonical Supabase during G2 2029 rulebook reconciliation on 2026-09-13 where changed. Unchanged rows retain their prior verified state until their domain is touched. Long-form contract/implementation claims still require live verification before any current-state write.
...
|---|---|---|---|---|
| One Reality Graph / One Tree identity + relations | `reality_graph_law` | versioned rule + codex | **v5 ACTIVE · DB-LIVE 2026-09-13** | `nodes/edges`; v5 extends the existing owner with qualified Research Anchors, Number/Expression/Concept separation, typed relations, dynamic clusters and projection semantics; no parallel graph/store |
...
| Research Plan / capability selection / Temporal World Model / evidence-aware AI research | `research_strategy_layer_law` | versioned rule | **v3 ACTIVE · DB-LIVE 2026-09-13** | Existing Research Plan owner extended with Typed Observation/Value Origin, evidence independence, multi-axis time/history, Negative Results, replay, Information-Gain planning, stopping rules, Compile Once → Project Many and Bundle-first composition where adapters exist. Does not own Truth or create a second research store |
...
| Agent coordination / owner creation / parallel write safety / 2029 fresh-agent entry | `inter_agent_coordination_law` | versioned rule | **v10 ACTIVE · DB-LIVE 2026-09-12** | `nodes(type='rule')`; `work_log` is coordination channel. v10 preserves v9 G1 entry safety and clarifies §60 applicability: governance/project-task-start adapters (GPT/CLAUDE/future project agents) require fresh project-entry replay; bounded product/domain runtimes such as Raziel/Metatron are verified at their real domain/runtime boundary and must not perform unrelated project bootstrap |
...
| Numeric / Gematria research routing and governed numeric-family expansion | **existing composition:** `project_codex.numeric_rule_family_index` + current numeric/method owners | routing composition, **not a new owner** | **INDEX LIVE · POINTER VERIFIED 2026-09-12** | `project_codex.numeric_rule_family_index` is the routing index that separates deterministic engine methods from System/ZURIEL numeric research laws and points inward to `canonical_methods_registry_law`, `engine_governance_registry_authority_law`, active numeric-law owners and Research Intake provenance. For queries such as 358, resolve this pointer plus current owners live; the index itself does not calculate, canonicalize, publish, or become a second registry |
...
| Foundation closure protocol | `foundation_closure_protocol_law` | versioned rule + codex | **v4 ACTIVE · DB-LIVE 2026-09-12** | v4 preserves Bedrock Ladder + mandatory G0–G8 semantics and binds G1 closure to the current `inter_agent_coordination_law` fresh-agent entry acceptance; it explicitly does not authorize G1 execution while G0 remains open; `project_codex.foundation_closure_protocol_v1` lineage |
