...
test("selected occurrence becomes axis layer without changing occurrence truth", () => {
  const out = projectEls2029Layers({
    ...
    occurrences: [
      { occurrenceId: "occ-1", corpusId: "torah-v1", corpusVersion: "mt-text-v1", skip: 7, dir: 1, start: 10, end: 31, coordinateConvention: "zero_based_character_index", positions: [10,17,24,31], dependencyGroup: "g1" },
      { occurrenceId: "occ-2", corpusId: "torah-v1", corpusVersion: "mt-text-v1", skip: 11, dir: -1, start: 90, end: 57, coordinateConvention: "zero_based_character_index", positions: [90,79,68,57], dependencyGroup: "g2" },
    ],
  });
  ...
  assert.equal(out.layers[0].corpusVersion, "mt-text-v1");
  assert.equal(out.layers[0].end, 57);
  assert.equal(out.layers[0].coordinateConvention, "zero_based_character_index");
...
test("layer cells preserve corpus positions but do not mint glyph identity or rendering truth", () => {
  const out = projectEls2029Layers({
    ...
    occurrences: [{ occurrenceId: "occ-a", corpusId: "torah-v1", skip: 9, dir: 1, start: 50, end: 68, coordinateConvention: "zero_based_character_index", positions: [50,59,68], dependencyGroup: "same-window" }],
  });
...
test("projection preserves dependency groups and refuses proximity/evidence inference", () => {
  const out = projectEls2029Layers({
    ...
    corpusId: "torah-v1",
    term: "אלהים",
    occurrences: [{ occurrenceId: "occ-x", corpusId: "torah-v1", skip: 4, dir: 1, start: 5, end: 21, positions: [5,9,13,17,21], dependencyGroup: "window-1" }],
  });
