...
### 4.12 HYBRID ACCESS LAW — new, added 22.8

> SOD1820 supports both subscription-based entitlements and one-time token/credit unlocks through **one unified access-control layer**. Feature truth/status remains independent of payment mechanism.

Direct extension of §4.7 (Premium Law) and §4.8 (composition with `platform_tiers_law`) — names a **payment-mechanism axis**, orthogonal to Classification/Provenance-Generation/Approval/Access (§4.3). `platform_tiers_law` (CLAUDE.md) already names both halves of this without formally unifying them: the 6-tier subscription ladder (guest → registered → temple-student → temple-son → temple-researcher → temple-partner) *and* **Sod Credits** ("מנוי = מכסה חודשית + אפשר לקנות עוד" — subscription is a monthly quota, credits are a one-time top-up, with an existing per-action price list: ELS search=10, AI report=25, cross=5, AI journey=30). This law is the explicit statement that these are **two entry-points into the same access-control layer**, not two competing systems.

**What this settles:**
- **One access-control layer, two unlock paths.** A feature/depth-level gated per §4.4–§4.7 can be unlocked either by holding the qualifying subscription tier *or* by spending the equivalent one-time token/credit amount. Neither path is a second, parallel entitlement system — both resolve to the same underlying access-check.
- **Feature truth/status ≠ payment mechanism.** Whether a user reached a feature via subscription-tier or via a one-time credit spend never changes the feature's own truth: the same engine-computed value, the same `verification_state` (§1), the same canonical status, regardless of *how* access was purchased. This extends §4.7's "Premium controls access... never mathematical truth or canonical status" by naming the *mechanism of access* explicitly as a further thing Premium/payment can never touch.
- **No parallel entitlement system.** Both mechanisms feed one access-control layer — never "subscription-users see the real system, credit-users see a lite version" or the reverse.

**Composes with, does not replace:** `platform_tiers_law`'s 6-tier ladder + existing gate order, and the existing Sod-Credits price list — both stay exactly as documented; this law is the unification statement between them, not a new tier, not a new currency, not a new gate order.

**Left open (not decided here, same disposition as §4.11):** the concrete implementation of "one unified access-control layer" (a single entitlement-check function consulted by both paths? a shared `profiles` row carrying both `tier` and `credits`, per the still-open `profiles` gap named in `RAZIEL_PERSONALIZATION_LAW.md` §2.4/§10 and `personal_command_center_law`?) — no schema, no RLS, no UI is proposed or built here. Per-feature credit-cost-vs-tier-equivalence mapping is also not decided — this section states the *architectural* unification only, not a pricing table.
