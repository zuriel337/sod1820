...
| `facebook-publish` | `true` | real JWT + canonical `rd_is_admin()` before service-role/provider mutation | repairs current git-only/live-client drift; direct one-post/one-image admin action remains distinct from queued `share-to-facebook` |
...
3. Apply `G0_EDGE_RELEASE_SQL_CANDIDATE.sql` through canonical Supabase migration action. This updates `agent_identity.raziel.wa_slug` to `wa-raziel`, replaces the `wa-raziel` cron with a Vault-backed header call, and replaces the Watchman HTTP cron with the DB runner.
4. Verify DB grants/functions/crons, especially `system_watchman_run`, `notify_payment_request_tg`, `detect_suggestions`, `site_pulse`, the Raziel identity row, and the G0-targeted Vault-backed cron headers.
5. Deploy hardened KEEP/HARDEN candidates with the exact gateway modes above. `wa-raziel` must be live and accept the Vault-backed header path before `wa-christina` is tombstoned. `facebook-publish` must reject unauthenticated/non-admin invocation and succeed only for a real admin session.
...
7. Deploy 410 tombstones only after their replacement/DB path is live where applicable. In particular, retire `wa-christina` only after `wa-raziel` + `agent_identity` verification.
8. Negative auth replay: unauthenticated privileged/cost/mutation paths must fail; authorized paths must still work.
9. Re-scan active cron commands. The independent audit found additional legacy inline credentials outside the G0-targeted cron set; these are **not silently treated as fixed** by this matrix. They require either bounded owner-by-owner remediation or an explicit Human-Gate accepted P1 deferral before G0 closure.
10. Only then merge the matching git representation on explicit release authorization and run the full G0 closing rescan.
