...
async function graph(path: string, opts: { method?: string; token?: string; params?: Record<string, string> } = {}) {
...
// מזהה IG: או ישיר (ig_id) או דרך הדף המקושר.
async function resolveIg(body: any): Promise<{ igId: string; token: string }> {
  if (body.ig_id) {
...
Deno.serve(async (req) => {
  ...
  _pages = null; // מטמון טרי לכל בקשה
  ...
  try {
    switch (action) {
      // ── אבחון: מגלה את כל הנכסים שמשויכים לטוקן ── //
      case "whoami": {
        const me = await graph(`me`, { params: { fields: "id,name" } }).catch((e) => ({ error: String(e) }));
        const ps = await pages().catch((e) => { throw new Error(`pages: ${e}`); });
        let ads: any[] = []; let adsErr = "";
        try {
        ...
        return json({
          ok: true,
          token_identity: me,
          pages: ps.map((p: any) => ({ id: p.id, name: p.name, instagram: p.instagram_business_account || null })),
          ad_accounts: ads,
      ...
      case "publish_post": {
        ...
        const params: Record<string, string> = { message }; if (body.link) params.link = String(body.link);
      ...
      case "ig_publish": {
        ...
        const url = String(body.image_url || "").trim();
        if (!url) return json({ ok: false, error: "image_url required" }, 400);
        // שלב 1: יצירת container; שלב 2: פרסום
      ...
      case "ads_set_budget": {
        ...
        if (!params.daily_budget && !params.lifetime_budget) return json({ ok: false, error: "daily_budget or lifetime_budget required (אגורות)" }, 400);
      ...
      case "ads_create_campaign": {
        ...
        const objective = String(body.objective || "").trim(); // e.g. OUTCOME_TRAFFIC, OUTCOME_ENGAGEMENT
