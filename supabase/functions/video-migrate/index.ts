...
// Fetches remote source video and self-hosts it in the media bucket.
// G0 hardening: fail closed unless OCR_RUN_KEY is configured and supplied.
...
function validDest(dest: string) {
  return !!dest && !dest.startsWith("/") && !dest.includes("..") && dest.length <= 500;
}
...
Deno.serve(async (req: Request) => {
  try {
    ...
    for (const item of items) {
      ...
      try {
        if (!validSource(src) || !validDest(dest)) {
          out.status = "invalid_input";
          results.push(out);
          continue;
        }
        if (await existsInStorage(dest)) {
    ...
    const summary = {
      ...
      failed: results.filter((r) => String(r.status).includes("fail") || r.status === "error" || r.status === "invalid_input" || String(r.status).includes("too_small")).length,
    };
