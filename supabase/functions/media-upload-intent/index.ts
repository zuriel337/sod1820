...
import { buildUploadIntent, mayVerifyPath, normalizeMime } from "./contract.ts";
...
async function issueIntent(actor: any, body: any) {
  ...
  const { data, error } = await actor.admin.storage
    .from(intent.bucket)
    .createSignedUploadUrl(intent.path, { upsert: false });
  ...
  return {
    ...
    tus: {
      ...
      metadata: {
        ...
        cacheControl: intent.scope === "public" ? "3600" : "0",
      },
    },
  };
}
...
async function verifyIntent(actor: any, body: any) {
  ...
  if (!bucket || !mayVerifyPath({
    scope,
    path,
    userId: actor.userId,
    contributorId: actor.contributorId,
    isAdmin: actor.isAdmin,
  })) throw new Error("verify_forbidden");
  ...
  const checks = {
    ...
    mime_match: actualMime === expectedMime,
  };
  return {
    ok: checks.size_match && checks.mime_match,
    action: "verify",
    scope,
    bucket,
    path,
    expected: { size: expectedSize, mime: expectedMime },
    actual: { size: actualSize, mime: actualMime },
    checks,
  };
}
