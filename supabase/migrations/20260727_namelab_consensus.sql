-- ============================================================
--  NameLab — Advanced #1: multi-engine CONSENSUS (overlay)
--  Zuriel's law: consensus is NOT another engine — it is an OVERLAY that
--  counts how many INDEPENDENT method-families landed on the same finding
--  (a word or a verse). Sub-methods within a family count once. It never
--  counts itself. "הפסוק שלך" (fn_name_verse) is EXCLUDED from the score.
--  Families — word: גימטריה(ערך שווה) · שכנות(שכן בתנ״ך) · שורש · וריאציה.
--            verse: מילולי · גימטריה(ערך הפסוק) · צורת-אותיות(ר״ת/ס״ת).
--  A finding surfaces only with ≥2 independent families. Provenance kept.
--  Supersedes fn_name_multi from earlier migrations (adds `consensus`).
--  Example: יעקב → «ישראל» (וריאציה + שכנות) = genuine convergence.
-- ============================================================
...
declare
  ...
  root_first jsonb; root_last jsonb; nota jsonb; sv_words jsonb; neigh jsonb;
...
begin
  ...
  into v_consensus
  from (
    select vtype, target, count(distinct family) c, jsonb_agg(distinct family) families, jsonb_agg(distinct method) voters
    from votes where target is not null and target<>''
    group by vtype, target having count(distinct family) >= 2
    order by count(distinct family) desc limit 15
  ) z;
