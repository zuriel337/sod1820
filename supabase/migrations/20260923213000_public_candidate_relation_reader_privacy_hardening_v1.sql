...
--      SECURITY INVOKER default. The only reason it was ever made DEFINER was
--      to reach research_objects; edges/topic_cards are (and were already)
--      readable by anon/authenticated directly, so no elevated privilege is
--      needed once research_objects is no longer touched here.
--
...
COMMENT ON FUNCTION public.fn_relation_independent_evidence(text, text) IS
  'Relation Engine v1 — independent evidence (edges/topic_cards), read-only, SECURITY INVOKER. research_objects is never surfaced here (hardened 23.9.2026, PUBLIC_CANDIDATE_RELATION_READER_PRIVACY_HARDENING_V1): privacy_scope=''public_candidate'' is a research classification, not publication authorization, and research_objects has no live published/public projection to read from instead. The key is kept in the return shape as an always-empty array for caller compatibility (fn_relation_candidate). Internal/server research over research_objects continues through existing RLS-governed/service-role paths, untouched by this function.';
