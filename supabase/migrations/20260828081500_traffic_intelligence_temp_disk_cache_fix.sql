...
--
-- REPRODUCIBILITY-ONLY PORT (fresh-verified 2026-09-01, actor=CLAUDE, Clean Traffic
-- Classification Foundation v1 implementation pass):
...
-- commit 27aa52d3, PR #222 — never merged to main, re-verified still unmerged on
-- 2026-09-01 against origin/main 7274a8b1). This file is a byte-for-byte capture of
-- that already-live code, re-verified via whitespace-normalized md5 against
-- pg_get_functiondef() for all three functions on project linswmnnkjxvweumprav on
-- this fresh branch (hashes identical to the first verification pass on
-- 2026-08-31: admin_entries_breakdown=5c85effc..., admin_funnel=79ef7b31...,
-- admin_traffic_insights=5c8d7a3d... — all matched exactly, both times). Porting
-- this file changes NOTHING functionally — it only closes the git/DB drift so
-- `main`'s migration history reflects live reality. No classification/bot-detection
-- semantics touched; Behavioral Bot v3 (fn_human_entrances) untouched — verified
-- separately unchanged (hash 1a2e30ce...) in this same pass.
