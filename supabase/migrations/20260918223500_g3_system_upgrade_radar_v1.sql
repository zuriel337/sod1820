-- G3_SYSTEM_UPGRADE_RADAR_V1 — EXTEND_EXISTING system_suggestions_law v2 / system-watchman.
--
...
--   * Outbound requests target exactly one allowlisted host: https://registry.npmjs.org — no other
--     domain is ever constructed, and no arbitrary page is fetched/scraped.
...
--   * Node engine: package.json carries no "engines.node" pin to compare against, so a Node check
--     would have to invent a "current" value. Left out of the allowlist rather than fabricated;
--     wiring it in later needs an explicit current-Node-version source first.
...
--
-- Release: BRANCH_ONLY_NO_MERGE_NO_DEPLOY_NO_LIVE_APPLY per work_log b514fadd-00c3-4e22-81d0-
-- f6fc01fa4352. Not applied to canonical Supabase linswmnnkjxvweumprav by this session.
