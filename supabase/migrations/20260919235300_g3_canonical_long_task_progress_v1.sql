...
begin
  ...
  values (
    ...
    coalesce(v_prev.description,'') || E'\n\n'
      ...
      || '11. DEFAULT PRESENTATION TIMING. ה-primitive שומר slot מיד; סביב 3s הוא רשאי לפתוח הסבר של שלבים, וסביב 12s שכבת long-wait companion. Domain שיודע מראש שהפעולה כבדה רשאי לפתוח long form מהתחלה. thresholds הם presentation בלבד, לא operational truth.' || E'\n'
      || '12. MIGRATION. זהו forward law ל-2029/new/redesigned surfaces. Legacy loaders עוברים אליו כשה-surface שלהם נכנס redesign או כשה-loader משתנה מהותית; אין mass rewrite עיוור.',
    v_metadata,
    true,
