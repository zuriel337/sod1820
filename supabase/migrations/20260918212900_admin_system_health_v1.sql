...
--
-- Owners: system_suggestions_law v2 + subscription_funnel_law v19 +
...
--     public.notify_admin() per subscription_funnel_law v19, instead of a direct
...
comment on function public.admin_system_health() is
  'G3_2029_CONTROL_PLANE_FOUNDATION_V1 — admin/service-only bounded read projection (system_suggestions_law v2 + subscription_funnel_law v19 + experience_governance_foundation_v1_law v6 + research_intake_foundation_contract_law v11 + foundation_closure_protocol_law v6). Bounded, privacy-safe operational facts only: no cron command text or secrets, no recipient addresses, no object paths, no raw private content, no universal score. Retention is a pointer/summary of admin_retention_preview(), never duplicated. Fail-closed: service_role or rd_is_admin() only.';
...
begin
  ...
  if array_length(v_reasons,1) is not null then
    ...
    if not exists (select 1 from work_log
      ...
      -- כל התראה מסתיימת ב-notify_admin (subscription_funnel_law v19) — עטוף: כשל בשליחה לא שובר את הניטור
      begin
...
comment on function public.fn_health_watch() is
  'G3_2029_CONTROL_PLANE_FOUNDATION_V1 correction — infrastructure health alerts terminate in public.notify_admin() per subscription_funnel_law v19. No direct wa_send()/hardcoded WhatsApp target. 1-hour work_log dedupe and failure-isolation preserved.';
