-- ============================================================
--  NameLab — Advanced #5 (milui) + #7 (anagrams) + evidence law
--  Zuriel's guardrails:
--   • milui returns the fill-value + expressions of the SAME value with
--     their source + count + objective ranking (by length). These are
--     "value matches", NOT the meaning of the name — interpretation stays
--     with Metatron/the user.
--   • anagrams: ONLY corpus-verified words (same letter multiset), each
--     classified by type (מקרא / מילון). Never invents words.
--   • EVIDENCE LAW: every track carries an `evidence` level —
--     direct (Tanach appearance / computed / verified anagram) ·
--     value_match (same-gematria expression) ·
--     interpretive (Metatron/AI connection over several findings).
--  Wired into fn_name_multi as tracks 'milui' + 'anagrams'; every track
--  now has `evidence`, and the doc carries an `evidence_legend`.
--  Supersedes fn_name_multi (final form with both tracks + evidence).
-- ============================================================
create or replace function public.fn_miluy_engine(p_parts text[])
returns jsonb language sql stable set search_path=public as $$
  select jsonb_build_object('parts', coalesce((select jsonb_agg(jsonb_build_object(
    'part', p, 'ragil', fn_ragil(p), 'milui', fn_miluy(p),
    'match_count', (select count(*) from gematria_words where ragil=fn_miluy(p) and phrase<>p),
    'matches', (select coalesce(jsonb_agg(jsonb_build_object('phrase',phrase,'source',coalesce(source,'—'),'len',char_length(phrase))
                  order by char_length(phrase), phrase),'[]'::jsonb)
                from (select phrase, source from gematria_words where ragil=fn_miluy(p) and phrase<>p order by char_length(phrase), phrase limit 8) z),
    'evidence','value_match'
  )) from unnest(p_parts) p where p<>''),'[]'::jsonb));
$$;
grant execute on function public.fn_miluy_engine(text[]) to anon, authenticated;

create or replace function public.fn_anagrams_engine(p_parts text[])
returns jsonb language sql stable set search_path=public as $$
  with base as (
    select p, translate(regexp_replace(p,'[^א-ת]','','g'),'ךםןףץ','כמנפצ') b,
      (select string_agg(c,'' order by c) from regexp_split_to_table(translate(regexp_replace(p,'[^א-ת]','','g'),'ךםןףץ','כמנפצ'),'') c) sig
    from unnest(p_parts) p where p<>''
  )
  select jsonb_build_object('parts', coalesce((select jsonb_agg(jsonb_build_object(
    'part', base.p, 'evidence','direct',
    'anagrams', (select coalesce(jsonb_agg(distinct jsonb_build_object(
        'word', gw.phrase,
        'type', case when exists(select 1 from tanach_verses tv where tv.words @> array[gw.phrase]) then 'מקרא' else 'מילון' end)),'[]'::jsonb)
      from gematria_words gw
      where gw.phrase <> base.b and char_length(gw.phrase)=char_length(base.b)
        and (select string_agg(c,'' order by c) from regexp_split_to_table(translate(regexp_replace(gw.phrase,'[^א-ת]','','g'),'ךםןףץ','כמנפצ'),'') c)=base.sig)
  )) from base),'[]'::jsonb));
$$;
grant execute on function public.fn_anagrams_engine(text[]) to anon, authenticated;
